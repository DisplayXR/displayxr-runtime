...
This breaks when a machine has multiple 3D displays from different vendors (e.g., one vendor on monitor A, a different vendor on monitor B), or when a developer wants to force `sim_display` on one specific monitor while using the real vendor DP on another.

Investigation of one vendor SDK revealed that it internalizes multi-monitor handling: it splits a window into 3D/2D regions per monitor, auto-detects whether the window is on a 3D display, intercepts window positioning for phase alignment, and runs background polling threads to monitor display configuration changes — with no flag to disable any of this behavior. A vendor SDK that does this cannot coordinate routing across vendors.

Two options were evaluated:

- **Option A — Vendor SDKs handle it**: Each SDK manages its own display detection. Problem: vendor SDKs only know about their own displays and cannot coordinate routing across vendors. Also prevents configurations like sim_display-on-monitor-A + vendor-DP-on-monitor-B.
...
1. **Vendor display probe interface**: Each vendor driver provides a `probe_displays()` function that identifies which OS monitors it recognizes as its hardware. Vendors use proprietary detection methods — e.g., a vendor may use EDID matching (manufacturer+product ID) combined with a hardware USB handshake that provides device serial numbers via shared memory. DisplayXR does not need to understand these mechanisms; it only needs the result: a list of claimed monitors with confidence levels.
...
6. **Per-display eye tracking via DP vtable**: Camera-to-display pairing is vendor-specific hardware (e.g., a vendor may bundle display + camera + calibration as one unit). DisplayXR does not manage this pairing. Each DP instance is already per-display, and `get_predicted_eye_positions()` on the DP vtable returns eye positions calibrated to that specific display. The vendor probe must include enough identity (e.g., a hardware serial) for the DP factory to create a DP tied to the correct camera/calibration. In multi-display setups, this means multiple simultaneous eye tracking streams — one per DP.
7. **HWND ownership model**: DisplayXR determines which display has the majority of the window area (the "primary" display). Only that display's DP receives the real HWND at creation time via the `window_handle` factory parameter. Secondary display DPs receive `window_handle = NULL`. Vendor DPs must handle both cases:
   - **HWND provided** → DP owns phase snapping via WndProc, but disables internal monitor polling, canWeave, and getDrawRegions (DisplayXR handles routing).
   - **HWND = NULL** → DP must not hook WndProc or call MonitorFromWindow. Phase is computed entirely from `canvas_offset_x/y` in `process_atlas()`.
8. **Phase alignment and window drag**: On Windows, DWM composites the last weaved frame at new positions during drag without re-running the shader. If the window moves by even 1 pixel, the interlaced pattern misaligns with the lenticular lens → crosstalk. Vendor SDKs handle this by hooking `WM_WINDOWPOSCHANGING` to constrain window movement to lens-pitch-aligned positions (phase snapping). This is proprietary per vendor — DisplayXR does not need to know the grid parameters, it just gives the HWND to the primary DP and lets the vendor handle it.
   - **Multi-display drag**: During drag across a display boundary, the primary DP phase-snaps to its own display's grid. Secondary displays may have imperfect phase during drag — acceptable since dragging is transient. Once drag ends, the shader re-renders all regions with correct phase from canvas offsets.
   - **Primary display hot-swap**: When the window moves mostly onto a different display, DisplayXR destroys the old primary DP, creates a new one for the new primary display with the HWND, and demotes the old primary to a NULL-HWND secondary DP.
...
- Same-vendor multi-display works (e.g., two same-vendor monitors, force sim on one).
- Requires vendor SDK changes: DPs must accept `window_handle = NULL` (compute phase from canvas_offset only, no WndProc hook). This is simpler than the earlier EXTERNAL_ROUTING flag concept — HWND presence/absence controls behavior.
- Phase snapping remains vendor-owned and proprietary. DisplayXR only decides *which* DP gets the HWND — no phase grid information leaks across the vendor boundary.
