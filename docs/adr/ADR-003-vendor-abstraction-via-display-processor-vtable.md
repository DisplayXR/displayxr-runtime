...
## Context
Vendor-specific code (vendor `#ifdef` blocks) was scattered across compositor and state tracker files (~60 blocks). A new vendor would need to add their own `#ifdef` blocks to the same files.
...
## Consequences
Zero vendor `#ifdef` blocks in compositor or state tracker code. New vendors add files only under `src/xrt/drivers/<vendor>/` and `src/xrt/targets/common/`. All ~60 vendor-specific `#ifdef` blocks removed from core.
