...
| `GripSurface`, `GripSurface-XR_KHR_maintenance1` | These want `…/input/grip_surface/pose`. `grip_surface` **is** present in `src/xrt/auxiliary/bindings/bindings.json` as a virtual profile extending all five controller profiles, so the binding exists; whether the qwerty device actually backs the pose has not been verified on hardware. **If one of these fails, that is a defect to file against the runtime, not a judgement call.** Do not mark it FAIL-by-inspection; capture the output and open an issue. |
