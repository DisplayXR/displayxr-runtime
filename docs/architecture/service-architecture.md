...
| Linux: login → socket; client → service | **systemd user socket activation** (#1744). The `.deb` enables `displayxr.socket` for every user (`systemctl --global enable`; `postinst` also starts it in running user managers); it listens on `%t/displayxr_comp_ipc`, and the first client that dials it starts `displayxr.service`, which exits 30 s after its last client leaves (`IPC_EXIT_WHEN_IDLE`). The service reads the `LISTEN_FDS` hand-off itself (no libsystemd). Which clients dial is the hybrid router's decision: `XR_DXR_weave` present-owners when the socket exists (else in-process), workspace controllers, `XRT_FORCE_MODE=ipc`; everything else stays in-process | `scripts/linux/systemd/`; `ipc_server_mainloop_linux.c` (`listen_fds_from_env`); `targets/openxr/target.c` |
...
| supervision | orchestrator for children; nothing for the service | orchestrator | systemd user manager (`Restart=on-failure`; the socket re-starts it on the next connect) | the OS (`START_STICKY`, but restart arrives with a null Intent → not re-foregrounded, `MonadoService.kt:63-71`); no client reconnect |
