...
Last updated: 2026-03-31 (branch `feature/shell-phase0-ci`) — **PHASE 0 COMPLETE**
...
### Phase 0C.2: Two-app support (DONE)
- Slot-based window placement: slot 0 at (5%,5%,40%,40%), slot 1 at (55%,5%,40%,40%)
- All clients stay visible + focused in shell mode (multi-client focus override in IPC server)
- App HWNDs pushed to HWND_BOTTOM z-order (behind shell window, still visible to Windows)
- `run_shell_2apps.bat` generated by build script for quick two-app testing

### Phase 0D: Input forwarding (DONE)
- All keys go to both qwerty and focused app (ADR-014)
- Shell-reserved: ESC, TAB, DELETE only
- Mouse events forwarded 1:1 to focused app's HWND

## What's Next

See `shell-phase1-plan.md` for the next milestone:
- IPC-to-standalone hot-switch on shell dismiss
- Dynamic window poses (drag-to-move)
- Mouse-ray hit-test for window selection
- Shell app skeleton (privileged IPC client)
