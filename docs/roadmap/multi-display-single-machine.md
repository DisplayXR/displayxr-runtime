...
- **Sizing set:** currently-engaged displays, recomputed on overlap change, surfaced via the existing rendering-mode / recommended-dims change event (the app reallocates render targets exactly as it does on a `xrRequestDisplayRenderingModeDXR` mode switch) — realloc fires only on the rare first cross onto a *more-demanding* display. **Fallback:** static `max` over all connected DP displays (never reallocs; over-renders when parked on a low-demand monitor). Unlike view-*count* worst-casing (free per-frame — inactive tiles aren't rendered), tile-*resolution* worst-casing is paid **every frame**, so the engaged-set policy is preferred for GPU-bound apps.
