...
- **Open at the workspace layer.** `XR_DXR_spatial_workspace` defines how any controller process drives multi-app composition, hit-test, and capture; launcher tiles come from apps' `*.displayxr.json` manifests. The reference shell uses these surfaces; so does anyone else.
...
- **Runtime + custom controller.** Write your own workspace controller against `XR_DXR_spatial_workspace`. Differentiate your product line with a branded launcher, layout presets tailored to your form factor, custom chrome. Your controller gets the same first-class authentication as the reference shell.
...
- **Phase 2.A–2.H** (in progress): migrate launcher / chrome / capture / hit-test policy out of the runtime's compositor and into the workspace controller process, behind the as-yet-finalizing `XR_DXR_spatial_workspace.h` header.
