...
2. **Per-window Kooima:** enable the window-local rebase currently `#ifdef XRT_OS_MACOS` in `ipc_try_get_oop_view_poses` (`ipc_server_handler.c:1236-1258`) on Android, from the real rect (in-process: `comp_vk_native` metrics).
