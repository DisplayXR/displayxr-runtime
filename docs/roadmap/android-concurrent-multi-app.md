...
| F11 | **DONE (#1036).** `XR_DXR_weave` ships on Android: `comp_multi_weave_android.c` (AHardwareBuffer in/out, DP factory marshalled to the main thread, caller-published window geometry into the #1033 phase slot) + spec v7 (handle kinds, `xrWeaveBindWindow2DXR`). Hardware-proven on the NP02J with `test_apps/weave/weave_client_vk_android` — 5.8–6.7 ms per submit at 2560×1472, two rects, and a readback showing the CNSDK interlace lattice. The last risk — Chromium's GPU-process ↔ runtime connection — is **also answered (#1056)**: a process that already holds a connected socket adopts it (`ipc_client_connection_adopt_fd` / `DXR_IPC_FD`, [spec](../specs/runtime/android-ipc-fd-adoption.md)) instead of dialling, so the GPU process never touches Java, `bindService` or the runtime's classes. What remains is the browser port itself. | 🟢 |
...
**Flavors — GONE as of #1031.** There was a `deployment` flavor dimension: `inProcess` (`XRT_FEATURE_SERVICE=OFF`) and `outOfProcess` (`=ON`, adding `:src:xrt:ipc` and the CNSDK Java-glue AAR), mutually exclusive on a device because both claimed the `org.khronos.openxr.runtime_broker` authority. They are now **one hybrid APK** built with `XRT_FEATURE_SERVICE=ON` **and** `XRT_FEATURE_HYBRID_MODE=ON`; the deployment is picked per process in `target.c` (in-process default). CI builds `assembleDebug` (`build-android.yml`). In-process is intact: `comp_vk_native_target.cpp` Android backend (`:128-132`, `:1337-1360`, surface re-sync `:1649-1730`); runtime self-spawned SurfaceView when no window is bound (`oxr_session_gfx_vk_native.c:274-300`, "single-app POC"); plug-in loaded into the app via `target_plugin_loader.c` Android branch (`:983-1123`). No `XR_DXR_android_surface_binding` spec file exists (display_info.md §4 "planned").
...
|---|---|---|
| Binder to vendor services from an app | 🟢 **proven on device** (#1037) | `<queries>` matching by **intent action** grants *package-level* visibility (developer.android.com/training/package-visibility/declaring). Vendor-neutral convention: vendor services advertise `<intent-filter><action android:name="org.displayxr.action.VENDOR_DISPLAY_SERVICE"/>`; a `displayxr` AAR manifest declares one `<queries><intent>` for that action ⇒ no temporal/combinatorial coupling. **Verified on a retail NP02J:** with zero vendor package names in the shipped manifest (`aapt2 dump xmltree`), intent-only `<queries>` gave working `getPackageInfo`, core init and head tracking — **and an explicit-component `bindService` to the backlight service still succeeded**, the one check D5 called for. Stripping the `<package>` entries reproduced ADR-025's `NameNotFoundException` abort, so visibility genuinely applied. **Still requires a CNSDK/firmware manifest change** for the neutral action string (L7, in scope). |
...
| CNSDK host requirements | 🟢 **milder than assumed** (#1037) | Measured in-process, DP creation ran on the `native_app_glue` `android_main` thread (≠ the process main thread) and the core initialised fine — and `leia_cnsdk_create` is already non-blocking. The real requirement is **"the calling thread has a prepared `ALooper` that is pumped afterwards"**, which `native_app_glue` and a Java-UI-thread `xrCreateSession` both satisfy. The genuine gap is an app calling `xrCreateSession` on a bare pthread render thread (the engine shape) — then `android_main_thread_dispatch_init()` + async marshalling is needed. Activity-only calls already gated (`leia_cnsdk.cpp:589-618`). |
| CNSDK Java glue in the app classloader | 🟢 **solved** (#1037) | Core-impl load goes through `com.leia.sdk.internal.Plugin`/`com.leia.core.Loader` (sdk AAR), and the core `DexClassLoader` takes its **parent from the Context handed to `leia_core_library_load`** (`leia/common/android/classLoader.cpp`) — so the Context is the sole injection point. Shipped: `xrt_plugin_host_iface::get_android_class_host_context` returns a `createPackageContext(<runtime pkg>, INCLUDE_CODE\|IGNORE_SECURITY)` Context (carved from `reserved[]`, no ABI bump), the runtime APK carries the AAR (both flavors then; the one merged APK since #1031), and the plug-in uses it for the core load only (the Activity still serves `limit_orientations`). Result on hardware: a cube APK with **0 `com.leia.*` classes, 0 vendor `.so`, 0 vendor package names** weaving through the Leia DP in its own process. |
...
> **Spike result (2026-08-18, #1037).** Both packaging blockers are **solved and
> demonstrated on an NP02J**: intent-only package visibility works (including
> explicit-component `bindService` afterwards), and vendor Java glue is served from the
> runtime APK through the new `get_android_class_host_context` host-iface slot. A cube
> APK with zero vendor classes, `.so` or package names weaved through the Leia DP in its
> own process. The remaining A-specific blocker is **item 1 of #1037 — the
> runtime-spawned SurfaceView** — which crashes (`NullPointerException` in
> `SurfaceView.onAttachedToWindow`, no `ViewParent` to `requestTransparentRegion` on)
> the moment two apps are staged side by side, and the **in-process window-rect feed**
> (`canvas=0,0 0x0`), the in-process equivalent of #1033. See
...
> **AS BUILT (2026-08-19, #1031/#1037).** A is implemented and is now the **default**
> deployment. Vendor-neutrality: the plug-in's Java glue is served from the runtime APK
> through `xrt_plugin_host_iface::get_android_class_host_context` (ADR-036 D2 Am. 1), so
> an app APK carries zero vendor classes and zero vendor `.so`. Windows:
> `XR_DXR_android_surface_binding` is real (`xrSetAndroidSurfaceDXR` +
> `xrSetAndroidWindowGeometryDXR`, #1063), which unblocked multi-window A — two
> in-process cubes weave side by side at ~60 fps each with lens refcount 2. The last
> structural blocker, "only one runtime flavor installable at a time", is closed by the
> **flavor merge**: one APK carries the in-process compositor *and* the service +
> satellite slots, and `xrt_instance_create` chooses per process (`XRT_FORCE_MODE` /
> `debug.dxr.force_ipc`, a `com.displayxr.force_ipc` manifest `<meta-data>`,
> `XR_DXR_weave`, or an adopted `DXR_IPC_FD`). So A and C now coexist on one device,
> per application. Still open for A: L7, the vendor-neutral intent action (D5).
...
> **AS BUILT (2026-08-19, #1031).** The recommendation held, and the "keep C as well"
> half stopped being aspirational: **both deployments now ship in one runtime APK** and
> are selected per application at `xrt_instance_create`, not per device at install time.
> A is the default (in-process compositor + vendor core in the app's process); C is what
> a client gets when it forces IPC, declares `com.displayxr.force_ipc`, enables
> `XR_DXR_weave`, or adopts a service socket — all three of C's standing jobs (fallback,
> browser host, and now mixed-mode testing) are served without displacing A for
> everything else on the device. Design + rationale: ADR-036's flavor-merge amendment.
> B remains unpursued; #967d stays re-scoped to the workspace overlay mode.
...
4. **`XR_DXR_weave` on Android — DONE (#1036).** `comp_multi_weave_android.c` from the macOS template, un-gated in `oxr_extension_support.h`, spec v7 header: `XrWeaveSubmitHandlesDXR` handle kinds, and `xrWeaveBindWindow2DXR` + `XrWeaveWindowGeometryDXR` taking an explicit client-area origin + size instead of an HWND, which the runtime forwards to the #1033 `set_window_screen_rect` phase slot. Sync is macOS's: the caller finishes its writes before submitting and the submit returns only once the weave is GPU-complete — the `sync_file` acquire/release fd pair is a documented follow-up, a latency optimisation rather than a correctness gap. Two things the port exposed and fixed: the AHardwareBuffer IPC transport had no handle-count framing and `AHardwareBuffer_recvHandleFromUnixSocket` BLOCKS, so a receiver expecting a buffer the sender did not send hung rather than failed; and `comp_multi`'s render loop submitted on `vk->main_queue` without `vk_queue_lock`, which only became a real race once a weave client submits from its IPC thread.
...
5. **PoC-4 — browser weave. RUNTIME HALF DONE (#1036).** `comp_multi_weave_android.c` + the un-gated extension + `test_apps/weave/weave_client_vk_android`, a native present-owner submitting an AHardwareBuffer with rects, are on hardware: PRESENT_OWNER admission, weave engine bring-up, woven AHardwareBuffer handed back, window geometry reaching `leia_interlacer_set_viewport_screen_position`, and re-allocation across a rotation. **CONNECTION HALF DONE TOO (#1056).** The spike settled the shape: `Client.blockingConnect` from `:privileged_process0` is not worth building on (Java, a `Context`, cross-APK class loading, `<queries>` visibility, `bindService`), so the browser process connects and hands the fd down — `ipc_client_connection_adopt_fd(int fd)` (Mojo `PlatformHandle`) or `DXR_IPC_FD` (descriptor table at launch), validated and duped, consumed once by the next `ipc_client_connection_init()`. Proven on the NP02J by `weave_client_vk_android` under `debug.dxr.fdhandoff 1`, whose `:gpu` process owns the whole OpenXR/Vulkan/weave session while never calling `bindService`: 5.5–6.4 ms per submit vs the 5.8–6.7 ms in-process baseline — the handoff is free per frame. Spec, incl. the identity caveat (`SO_PEERCRED` names the *connector*) and the `RTLD_LOCAL` `dlsym` gotcha: [`android-ipc-fd-adoption.md`](../specs/runtime/android-ipc-fd-adoption.md). Running the weave client next to another app is unblocked by the satellite slots of #1053, which shipped. Remaining: the `displayxr-browser` port itself.
