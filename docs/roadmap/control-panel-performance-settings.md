...
| `DXR_WEAVE_OUTPUT_LINEAR` | `compositor/multi/comp_multi_weave_linux.c` (`weave_create_output_dmabuf`) | `DEBUG_GET_ONCE_BOOL` | off | Svc | 2 | Desktop-Linux XR_DXR_weave stage B (#1699): the woven dma-buf output's DRM modifier. Default = the driver picks from every modifier the service's device can export (on an Intel Xe2 iGPU a compressed CCS layout, which a same-device GL consumer re-imports bit-exact); `1` forces `DRM_FORMAT_MOD_LINEAR`, the one layout every importer (another GPU, the CPU) can read, at a bandwidth cost. Only the dma-buf output is affected; the stage-A OPAQUE_FD output is unchanged |
| `DXR_SPAN_2D` (alias `DXR_WAYLAND_SPAN_2D`) | `compositor/multi/comp_multi_weave_linux.c` (`weave_span_2d_enabled`); `compositor/vk_native/comp_vk_native_compositor.c` (`vk_composite_offpanel_2d`, Wayland) | hand-rolled `static`, `getenv` once | **on** (kill switch) | Both | 4 | #1654: the part of a window that is off the 3D panel is painted flat 2D (one view of the atlas) instead of woven — in-process on Wayland, and in the desktop-Linux `XR_DXR_weave` service engine (X11 and Wayland alike, from the caller-published geometry; a window wholly off the panel skips the weave). `0` weaves the whole window, the pre-#1654 behaviour. Diagnostic |
| `DXR_WEAVE_FLAT_2D` | `compositor/multi/comp_multi_weave_linux.c` (`weave_flat_2d_enabled`) | hand-rolled `static`, `getenv` once | **on** (kill switch) | Svc | 4 | XR_DXR_weave spec v8 on desktop Linux (#1699): the caller's flat regions (per-submit `XrWeaveSubmitFlatRegionsDXR` + the sticky `xrWeaveSetScreenFlatRegionsDXR` latch) are painted flat in the woven output — the input page 1:1 for a batch submit, the centre view for v6 — because no Linux DP has a per-region lens to wish to. `0` accepts and ignores them (fully woven), the pre-change behaviour and the literal Windows/§2c "pixels unaffected" contract. Diagnostic |
| `DXR_TEST_SPAN_2D_INSET` | same two sites | hand-rolled `static`, `getenv` once | off | Both | 4 | Test hook for the above: pretends the panel is N px narrower and shorter, so the off-panel path is reachable with no second monitor. Paints 2D over real panel pixels — never for users |
| `DXR_LINUX_WEAVE_SPLIT` | `compositor/multi/comp_multi_weave_linux.c` (`weave_run_frame`) | `DEBUG_GET_ONCE_BOOL` | **on** (kill switch) | Svc | 4 | Desktop-Linux XR_DXR_weave (#1699): for a DP that submits its own command buffer during `process_atlas`, flush the pre-weave batch first so the DP's batch runs after this frame's blits (the Android #1036 ordering fix). `0` records everything in one buffer. Diagnostic |
...
| `DXR_LAZY_TRANSPARENCY` | `compositor/vk_native/comp_vk_native_compositor.c` (`vk_lazy_transparency_setup`) | `DEBUG_GET_ONCE_BOOL` | **on** | App | 4 | Lazy transparency (desktop Linux): a transparency-capable session runs the DP's desktop capture, compose-under and alpha-gate only while its content carries alpha < 1. `=0` keeps them on for the whole session (the pre-slot behaviour), for A/B. Only engages when the DP implements `set_transparency_active` |
...
| `DXR_STEREO_CAMERA` | `ipc/server/ipc_server_stereo_camera.c` (`DEBUG_GET_ONCE_BOOL_OPTION`, read at camera-manager create) | on | Svc | 1 | ADR-043 admin/dev kill switch: `0` = the service enumerates zero stereo cameras (indistinguishable from "none", on purpose). The user-facing twin is the planned R3 "Share the 3D camera with apps" toggle — a privacy control, not a performance lever |
...
| `DXR_STEREO_CAMERA_DEV_ALLOW` | `ipc/server/ipc_server_stereo_camera.c` (`authorise_locked`, `DEBUG_GET_ONCE_BOOL_OPTION`) | off | Svc | ADR-043 **R1-only** stand-in for camera consent: stream start and calibration reads are REFUSED (`XR_ERROR_PERMISSION_INSUFFICIENT`) unless it is set in the SERVICE environment. Deleted when R3 lands the OS + DisplayXR consent path. Never a setting |
