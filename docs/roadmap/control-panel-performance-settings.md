...
| `DXR_SPLIT_SAME_ADAPTER_D3D11` | `compositor/xbridge/comp_split_gate.c` (`comp_split_gate_env_same_adapter_d3d11`; consulted by the in-process D3D11 tier's Stage A) | `getenv`, latched | off (bring-up) | App | 2 | ADR-039 Phase C bring-up: `=1` lets the D3D11 leg engage the same-adapter split (doubles as the Phase B reroute's hypothesis probe — a real D3D11 app on the d3d11 out arm). Collapses into `DXR_SPLIT_SAME_ADAPTER` on acceptance |
