...
| `DXR_HOLDER_STALL_MS` | `d3d11_service/comp_d3d11_service.cpp` (`dxr_holder_stall_ns`) | `getenv`, cached | 2000 | Svc | 3 | #939 holder liveness: an active `APP_HWND` holder that lands no present at all (paced or unpaced) this long yields the panel to a live present-owner instead of showing black. `0` disables |
| `DXR_HOLDER_RAISE` | `d3d11_service/comp_d3d11_service.cpp` (`dxr_holder_raise_enabled`) | `getenv`, cached | off | Svc | 4 | #939 A/B: raise a runtime-owned hosted window to the top (no activation) when it takes the panel. Refuted as a fix and suspected of causing a foreground flap on first hardware run; knob only |
| `DXR_HOLDER_STALL_DWELL_MS` | `d3d11_service/comp_d3d11_service.cpp` (`dxr_holder_stall_dwell_ns`) | `getenv`, cached | 3000 | Svc | 3 | How long a slot that yielded for stalling is barred from re-taking the panel; doubles per consecutive yield (cap 30 s), reset by a real grant |
