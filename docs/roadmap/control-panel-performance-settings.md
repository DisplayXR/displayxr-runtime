...
and tier. **72 distinct names**; the two `DXR_BG2D_*` knobs reach the environment through
...
| `DXR_WEAVE_REPAINT_GATE` | `auxiliary/util/u_repaint_gate.h` (`u_repaint_gate_open`, consumed by all four repaint loops) | `getenv`, cached in gate state | adaptive | App | 4 | `legacy` pins the pre-#1257 fixed 2-period quiet gate for A/B. Adaptive opens the repaint window after ONE missed vblank when the measured app cadence is stable and ≥ 1.5 periods (present-capped apps), and IS the legacy gate otherwise |
...
As of this writing that yields **72** names, all present above. Read sites and mechanisms
