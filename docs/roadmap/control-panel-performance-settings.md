...
and tier. **73 distinct names**; the two `DXR_BG2D_*` knobs reach the environment through
...
| `DXR_WEAVE_REPAINT_GATE` | `auxiliary/util/u_repaint_gate.h` (`u_repaint_gate_open`, consumed by all four repaint loops) | `getenv`, cached in gate state | adaptive | App | 4 | `legacy` pins the pre-#1257 fixed 2-period quiet gate for A/B. Adaptive estimates "app presents every N vblanks" and engages at N=2 only (kills the hz30 dead zone; governor-guarded); N≥3 and untrusted cadence deliberately fall back to legacy — five N≥3 schedule variants lost to it on hardware (#1257) |
...
As of this writing that yields **73** names, all present above. Read sites and mechanisms
