...
| `DXR_WEAVE_REPAINT_GATE` | `auxiliary/util/u_repaint_gate.h` (`u_repaint_gate_open`, consumed by all four repaint loops) | `getenv`, cached in gate state | adaptive | App | 4 | `legacy` pins the pre-#1257 fixed 2-period quiet gate for A/B. Adaptive estimates "app presents every N vblanks" (mode of round(interval/period)) and budgets N−1 repaints per app frame, presented clear of the app's FIFO queue slot; falls back to legacy without a trusted cadence |
