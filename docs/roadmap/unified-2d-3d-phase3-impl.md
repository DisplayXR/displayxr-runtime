...
- [x] Header v3 + xrt/oxr layer plumbing + event, macOS-green, Metal consumer validated locally (§8 cases on metal test apps). *(2026-06-07: **cases 2/3/4 — the headline `handle+mask+layer` — pixel-checked exact incl. §4.2 output-alpha (island weaves opaque, panel crisp, desktop `final.a=0` where neither covers); case-1 surround region byte-identical; view-size event fires at mask activation, silent for handle apps**. Metal Tier-3 = `XR_ERROR_FEATURE_UNSUPPORTED` (no Metal binding type in v3); the event struct carries the conventional `XrSession session` field the §3 sketch omitted; hosted (framebufferOnly) drawables skip the composite with a one-time WARN. **Known, deferred to the Windows D3D11 leg:** after rebasing onto #396-W7 (runtime now owns texture-app view generation from window metrics), the case-1 texture A/B has a residual canvas-region 3D scale offset — the old app-side world-scale-compensation point was deleted by #396, and reconciling a texture app's canvas-sized 3D with the runtime-owned window-metric view gen under an active mask is exactly where #396 + Phase 3 must meet; do it on the D3D11 leg with Leia validation. Compositor mechanism + handle path are correct.)*
