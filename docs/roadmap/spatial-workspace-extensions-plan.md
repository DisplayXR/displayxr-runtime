...
> **Superseded note (spec_version 22, issue #370):** the raycast hit-test did *not* stay in the runtime. It was later deleted and moved entirely to the controller; the runtime now feeds the controller the OS cursor position (on FRAME_TICK) and composites the cursor at the depth the controller pushes via `xrSetWorkspaceCursorDepthDXR`. See ADR-018's superseded note + `docs/architecture/separation-of-concerns.md`.
...
**Phase 2.K — controller-owned interactive layouts ✅ shipped.** Bumps `XR_DXR_spatial_workspace` to spec_version 6 with three new event variants on `xrEnumerateWorkspaceInputEventsDXR` (`POINTER_MOTION_EXT`, `FRAME_TICK_EXT`, `FOCUS_CHANGED_EXT`) and two new lifecycle requests (`xrRequestWorkspaceClientExitDXR`, `xrRequestWorkspaceClientFullscreenDXR`). The runtime no longer skips `WM_MOUSEMOVE` while pointer capture is enabled and now drives Win32 SetCapture/ReleaseCapture so drags survive cursor exits. The shell ports the deleted runtime carousel into a controller-side state machine — auto-rotation, drag-to-rotate, scroll-radius, TAB-snap, momentum — and adds a per-client animation framework that drives 300 ms ease-out cubic preset transitions and connect-time glides. Audit entry: [phase2-audit.md §Phase 2.K](spatial-workspace-extensions-phase2-audit.md).

**Phase 2.C — controller-owned chrome ✅ shipped.** Bumps `XR_DXR_spatial_workspace` to spec_version 7 with three new function PFNs (`xrCreate/Destroy/SetWorkspaceClientChrome*EXT`), three new structs (`XrWorkspaceChromeSwapchainCreateInfoDXR`, `XrWorkspaceChromeLayoutDXR`, `XrWorkspaceChromeHitRegionDXR`), one new enum value (`XR_WORKSPACE_HIT_REGION_CHROME_DXR`), one new field (`chromeRegionId`) on POINTER + POINTER_MOTION event payloads, and a no-longer-reserved POINTER_HOVER event. Runtime composites a controller-submitted chrome swapchain (cross-process SHARED_NTHANDLE + KEYEDMUTEX) at the controller-specified pose; ray-casts the chrome quad to fill `chromeRegionId` from the controller's declared hit regions. The shell ports the floating-pill design (rounded SDF shader, grip dots, close/min/max buttons, hover-fade ease-out cubic, click-dispatch to exit/fullscreen RPC). Final commit deletes the entire in-runtime chrome render block + focus-rim glow + fade machinery — runtime now ships with **zero default chrome**. App icon + DirectWrite glyph rendering deferred as a follow-up polish session. Audit entry: [phase2-audit.md §Phase 2.C](spatial-workspace-extensions-phase2-audit.md).

**Architectural endpoint.** With Phase 2.C shipped, the runtime owns zero pixels of UI policy in workspace mode. Mechanism (cross-process texture sharing, atlas composite, depth pipeline, hit-test plumbing, IPC) is the runtime's responsibility; appearance + interactivity policy is fully owned by the controller.

Output: two extension surfaces frozen and published — `XR_DXR_spatial_workspace` (the compositor-side extension: window pose, focus, capture, **per-frame motion** post-2.K) and `XR_EXT_app_launcher` (launcher tile registry, click events). Header sketches in [spatial-workspace-extensions-headers-draft.md](spatial-workspace-extensions-headers-draft.md).
...
- ~~Layout-preset semantics (`grid`, `immersive`, `carousel` from `comp_d3d11_service_apply_layout_preset`). The runtime today owns the math for each preset; in Phase 2 the workspace process computes per-window poses and pushes them via `workspace_set_window_pose`.~~ ✅ shipped in Phase 2.G — runtime no longer owns preset semantics; the shell ports the math and dispatches Ctrl+1..3 from the public input drain.
