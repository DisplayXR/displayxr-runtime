...
Two things outrank the floor, and there the under-submit clamp stands: a device that **pins** its mode (a dev/test override), and **service mode**, where the panel lease — not this session — owns the display-global mode. The runtime never silently clamps in either case: `xrBeginSession` logs `session in an UNFILLABLE rendering mode (#1499)`, and a later mode change into an unfillable mode logs once more as it lands. An app that wants the device's full width should begin on `PRIMARY_MULTIVIEW_DXR`.
