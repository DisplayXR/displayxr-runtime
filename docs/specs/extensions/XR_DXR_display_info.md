...
| `xrEndFrame` projection `viewCount` | **exactly 2** — the located count (ADR-041). An instance that enabled this extension may still submit 1 while the active mode is 1-view, **deprecated**, see below | **exactly the located count** (the device maximum). ADR-041 removed the old "any rendering mode's `viewCount`" allowance |
...
- **`xrEndFrame` rule — submit the located count, alias the inactive tail (v21, ADR-041).**
  ...
  **`DXR_UNDER_SUBMIT` — the staging switch.**
  ...
  | `1` **default** | the located count, **or** 1 while the active mode is 1-view → accepted, logged once | the located count |
...
| `VIEW_CONFIG_CANNOT_FILL_DXR` (v20) | the session's view configuration reports fewer views than the mode needs, so the session could not fill its tile grid — see below |
...
### A session never runs in, or requests, a mode it cannot fill (v20)

A session submits exactly as many views as the primary view configuration it **began** reports — 2 under `XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO`, the device max under `XR_VIEW_CONFIGURATION_TYPE_PRIMARY_MULTIVIEW_DXR` (v19). A rendering mode whose `viewCount` exceeds that has more tiles than the session can paint: the compositor's under-submit clamp paints the first ones and the per-frame clear leaves the rest flat. The runtime therefore does two things, both of which a `PRIMARY_MULTIVIEW_DXR` session never sees:

- **`xrBeginSession` applies a mode floor.** If the active mode cannot be filled, the runtime moves the display to one that can (preferring a 3D mode of exactly the session's width, then any fillable 3D mode, then the 2D mode) *before the first frame*, and reports it with `XrEventDataRenderingModeChangedDXR`. Apps that cached `isActive` from an enumeration made before `xrBeginSession` are therefore told; apps that re-enumerate see the floored mode.
- **`xrRequestDisplayRenderingModeDXR` denies a mode the session could not fill**, with `XR_DISPLAY_MODE_DENIAL_REASON_VIEW_CONFIG_CANNOT_FILL_DXR`. As for every other denial the call still returns `XR_SUCCESS`; the event is the answer, and the display does not move. The request is denied **locally**, before it reaches the panel-lease holder. A session that was created but never begun (a workspace controller, which drives the panel on behalf of its clients rather than painting into it) is not gated.

Two things outrank **both** of the above, and there the under-submit clamp stands: a device that **pins** its mode (a dev/test override), and **service mode**, where the panel lease — not this session — owns the display-global mode. In those cases the session is neither floored nor denied: a pinned session is left in its pinned mode, so refusing its request would be refusing it the mode it is already in (the device is the authority and answers for itself), and a service-mode client's request is **forwarded to the lease holder** rather than decided locally. The runtime never silently clamps in either case: `xrBeginSession` logs `session in an UNFILLABLE rendering mode (#1499)`, and a later mode change into an unfillable mode logs once more as it lands. An app that wants the device's full width should begin on `PRIMARY_MULTIVIEW_DXR`.
