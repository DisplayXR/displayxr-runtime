...
- `width` / `height` are the size of the **buffer** the runtime should attach, not the logical size from `xdg_toplevel.configure`. They are equal at desktop scale 1.0 and only there.
  ...
  - **Client-side decorations (#1654).** The bound surface must be the **content only**. Draw the title bar in a `wl_subsurface` above it, and make the window geometry (`xdg_surface.set_window_geometry`) bar + content. Then `xdg_toplevel.configure` reports the *frame* size, and the size to declare is the configure size **minus the bar**, times the scale as above. Never draw the chrome into the swapchain. It would sit inside the canvas and the atlas, and the weave would cover it. The runtime locates the surface through the geometry service's `buffer` rect, not the `frame`, so a bar above the surface does not shift the weave phase (see `docs/specs/runtime/wayland-window-geometry.md` §5). displayxr-common's `displayxr::linux_window` is the reference (`common/linux/dxr_wl_chrome.{h,cpp}`).
...
6. **Go fullscreen on the panel — and match the output in DEVICE pixels.** `xdg_toplevel_set_fullscreen(<wl_output>)` is the INV-1.3 substitute; a Wayland client cannot position itself. The match is where this goes wrong, because `wl_output.geometry(x,y)` is **logical** while the panel rect the runtime reports (`XrDisplayDesktopPositionDXR` `left/top` + `XrDisplayInfoDXR` `displayPixelWidth/Height`) is **device pixels**. Comparing them directly is a logical-vs-device comparison that simply cannot succeed on a scaled desktop: on 2026-09-20 no `wl_output` matched the panel rect `3840x2160+3456+0`, `set_fullscreen(NULL)` let the compositor choose, and it chose the laptop. The recipe:
   - Bind `zxdg_output_manager_v1` and take each output's **logical** rect from `zxdg_output_v1.logical_position` / `logical_size`. (`xdg-output-unstable-v1` is not in `wayland-protocols`' stable set; vendor the XML — the reference client keeps it at displayxr-common `common/linux/wayland-protocols/xdg-output-unstable-v1.xml`.)
...
- **Test app:** `test_apps/cube_handle_vk_linux` — one binary, both backends, `--platform=x11|wayland|auto` (or `DXR_WINDOW_BACKEND`; `--backend=` is the older spelling). The Wayland client is the shared helper `displayxr::linux_window` from displayxr-common (`common/linux/dxr_linux_window.{h,cpp}`), which implements every requirement in §4 and is the thing to copy. `test_apps/cube_zones_vk_linux` uses the same helper. Build with `scripts/build_linux.sh --apps`; the xdg-shell and `xdg-output-unstable-v1` glue is generated by `wayland-scanner` from the XML vendored at displayxr-common `common/linux/wayland-protocols/` (the helper needs xdg-output for the device-pixel output match in §4.6).
