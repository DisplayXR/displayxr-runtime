...
| **Leia SR display processor cannot weave a Wayland surface on LeiaSR 1.37** | The plug-in hands the SR weaver a native window handle; on this path there is none (`window = 0`), and the Linux Vulkan weaver in `leiasr-runtime` 1.37 responds `Window handle is invalid in VulkanWeaver::setWindowHandle: weaving is disabled.` — it refuses to weave without a window even though `srWeaverSetPresentOrigin` could supply the phase origin (the null-window gate is Win32 heritage; the Linux weaver never reads the window's position). Vendor-side change requested: [LeiaSR PR #224, comment 5749033439](https://github.com/LeiaInc/LeiaSR/pull/224#issuecomment-5749033439) (weave without a window when a present origin was supplied). Until it ships, Leia weaving on Linux is X11/XWayland only; `sim_display` presents normally on this path | Native-Wayland apps get a session and frames, but no lenticular weave from the Leia DP |
