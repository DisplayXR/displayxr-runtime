...
`XR_EXT_display_zones` lets an app compose, within its window, **N 3D zones**
...
- **3D framing** per zone = the `XR_EXT_view_rig` descriptors
  (`XrDisplayRigEXT` / `XrCameraRigEXT`), chained per-locate exactly as today;
  the new `XrDisplayZoneEXT` on the same locate scopes the framing to the
  zone's rect (the rect *is* the canvas).
- **2D zones** = `XrCompositionLayerLocal2DEXT` from `XR_EXT_local_3d_zone`,
  verbatim.
- **Explicit wish mask** = `XrLocal3DZoneMaskEXT` and its three authoring
...
`XR_EXT_view_rig` frames exactly one canvas, and the #439 Phase 2 supersede
...
### 3.2 XrDisplayZoneCapabilitiesEXT
...
### 3.3 XrDisplayZoneEXT — the dual-chain-point zone declaration
...
### 3.4 XrDisplayZonesFrameEndInfoEXT — per-frame wish reference
 ...
 * XrEventDataLocal3DZoneViewSizeChangedEXT (which is single-size and cannot
...
`XrDisplayZoneEXT` chain. Rules, all per ADR-027:
...
7. **Zero-zone frames** behave per `XR_EXT_local_3d_zone` v3 verbatim —
...
8. **Workspace mode**: a workspace client's wish is ignored in v1 (the
   ...
   `xrSetWorkspaceViewRigEXT` overrides substitute rig tunables on every
...
| `XrEventDataLocal3DZoneViewSizeChangedEXT` | `XrEventDataDisplayZoneMetricsChangedEXT` + per-zone re-query | Yes — old event keeps firing for legacy sessions |
| App-side Kooima (avatar v0.1.0) | Zone-scoped locate + `XrDisplayRigEXT` | N/A — the point of the design |
