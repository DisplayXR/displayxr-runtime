# Vendor Integrations

DisplayXR is vendor-agnostic — any 3D-display vendor can integrate their hardware by implementing the [display processor interface](../specs/vendor/display-processor-interface.md) and following the [vendor plug-in onboarding guide](../guides/vendor-plugin-onboarding.md).

This directory is the **vendor index**. Vendor implementation internals live with the vendor's code: plug-in vendors document themselves in their own repo's `docs/` (per ADR-019 — vendor code ships from vendor repos); only `sim_display`, which ships in this tree, keeps its docs here. Generic contracts that every vendor must follow live in `specs/vendor/` and `guides/`.

## Integrated vendors

| Vendor | Driver | Docs | APIs supported |
|---|---|---|---|
| **Leia SR** | plug-in ([displayxr-leia-plugin](https://github.com/DisplayXR/displayxr-leia-plugin)) | [displayxr-leia-plugin/docs/](https://github.com/DisplayXR/displayxr-leia-plugin/blob/main/docs/README.md) — weaver, transparency, chroma-key, phase snapping, mode switching | D3D11, D3D12, OpenGL, Vulkan |
| **sim_display** | `src/xrt/drivers/sim_display/` | [sim_display/](sim_display/) | D3D11, D3D12, OpenGL, Metal |

`sim_display` is the reference simulation vendor — it ships with the runtime and renders side-by-side / anaglyph output to a normal 2D window so contributors can develop without 3D-display hardware.

## Adding a new vendor

1. Read [guides/vendor-plugin-onboarding.md](../guides/vendor-plugin-onboarding.md) — the end-to-end walkthrough.
2. Implement the [display processor vtable](../specs/vendor/display-processor-interface.md) for each graphics API you support.
3. Decide your [eye-tracking mode](../specs/vendor/eye-tracking-modes.md) (MANAGED or MANUAL).
4. Document your internals (weaver, transparency model, etc.) in your plug-in repo's `docs/`, then add a row to the table above linking to it (docs-only PR against this repo).
5. The relevant ADRs are [ADR-003](../adr/ADR-003-vendor-abstraction-via-display-processor-vtable.md) (vendor isolation), [ADR-007](../adr/ADR-007-compositor-never-weaves.md) (compositor/DP boundary), and [ADR-015](../adr/ADR-015-displayxr-owns-multi-display-vendor-routing.md) (multi-display routing).
