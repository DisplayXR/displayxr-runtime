...
2. **Region paradigm — *how a mixed 2D/3D layout is expressed*.**
   Declarative [display-zones](../specs/extensions/XR_DXR_display_zones.md)
...
**submitting layers**, independent of app class — `XR_DXR_local_3d_zone`:

- **3D layer** — your normal projection layer (the weaved stereo content).
- **2D layer** — `XrCompositionLayerLocal2DDXR`, a post-weave screen-space 2D
...
- **3D-ness mask** `M ∈ [0,1]` — a *separate scalar channel* (not the 2D
  ...
  per-region 2D/3D ([`XR_DXR_display_zones`](../specs/extensions/XR_DXR_display_zones.md)).
...
API: [`XR_DXR_local_3d_zone.h`](../../src/external/openxr_includes/openxr/XR_DXR_local_3d_zone.h).

For **HUD-style 2D over the whole weave** (status bars, overlays in window-pixel
space rather than a masked region), submit `XrCompositionLayerWindowSpaceDXR`
...
- **Building a native app with your own window?** Use **Handle**. You create and manage the window, pass the handle (HWND, NSView) to the runtime via `XR_DXR_win32_window_binding` or `XR_DXR_cocoa_window_binding`. Most control, best for apps that need to own their window lifecycle.
...
- **Need the app, engine, or browser to own the swapchain?** Use **Texture** — for present-ownership: offscreen rendering, frame capture/streaming, or an engine/browser that must hold its own surface. You provide a shared texture; the DP weaves into it and you present it. You *should* also pass an on-screen HWND — it's the DP's position/phase anchor (drag phase-snap + canvas-scoped Kooima), **not** a render target, and it is optional in code (see [above](#the-hwnd-is-a-positionphase-anchor--and-for-texture-it-is-already-optional)) — but a fully windowless producer degrades until [#697](https://github.com/DisplayXR/displayxr-runtime/issues/697) lands.
  ...
  Full contract: [`XR_DXR_display_zones`](../specs/extensions/XR_DXR_display_zones.md); window binding: [`XR_DXR_win32_window_binding`](../specs/extensions/XR_DXR_win32_window_binding.md).
...
mixed-2D/3D path — `XrCompositionLayerWindowSpaceDXR` layers over the weave —
rather than a different app class.

## Further Reading

- [XR_DXR_local_3d_zone](../../src/external/openxr_includes/openxr/XR_DXR_local_3d_zone.h) + [unified 2D/3D compositing](../roadmap/unified-2d-3d-compositing.md) — the Local2D layer + 3D-ness mask (recommended way to mix 2D and 3D)
- [XR_DXR_display_zones](../specs/extensions/XR_DXR_display_zones.md) — hardware per-region 2D/3D (switchable-lens cells) driven by the same mask
- [In-Process vs Service](../architecture/in-process-vs-service.md) — detailed comparison of the two compositor deployment modes
- [XR_DXR_win32_window_binding](../specs/extensions/XR_DXR_win32_window_binding.md) — Win32 window binding spec
- [XR_DXR_cocoa_window_binding](../specs/extensions/XR_DXR_cocoa_window_binding.md) — macOS window binding spec
