...
# DisplayXR-authored extension names that are PLANNED (spec'd in docs but no
# header / no code symbols yet). Renamed by the name-based rules only — no
# header harvest. Keeping them under EXT in docs would promise a future
# namespace squat.
PLANNED = [
    "android_surface_binding",
]
...
EXCLUDE_DIRS = {
    ...
    "DerivedDataCache", ".gradle", ".idea", ".vs", ".venv", "venv",
}
...
def harvest_symbols(runtime_root):
    ...
    symbols = set()
    # Harvest both suffix forms so `map` regenerates correctly pre- AND
    # post-rename (post-rename headers carry DXR symbols; the map is always
    # keyed by the OLD EXT form).
    pats = [
        re.compile(rf"{LB}(Xr[A-Za-z0-9]+(?:EXT|DXR)){LA}"),      # types
        re.compile(rf"{LB}(PFN_xr[A-Za-z0-9]+(?:EXT|DXR)){LA}"),  # PFN typedefs
        re.compile(rf"{LB}(xr[A-Za-z0-9]+(?:EXT|DXR)){LA}"),      # functions
        re.compile(rf"{LB}(XR_[A-Z0-9_]+_(?:EXT|DXR)){LA}"),      # enums/defines
    ]

    def to_ext_form(sym):
        if sym.endswith("_DXR"):
            return sym[:-4] + "_EXT"
        if sym.endswith("DXR"):
            return sym[:-3] + "EXT"
        return sym
    ...
    for path in headers:
        ...
        for pat in pats:
            for m in pat.finditer(text):
                symbols.add(to_ext_form(m.group(1)))
...
def build_map(runtime_root):
    ...
    return {
        ...
        # Name-based rules (and verify) cover shipped + planned names; only
        # shipped names have headers to harvest symbols from.
        "allowlist": ALLOWLIST + PLANNED,
        "shipped": ALLOWLIST,
        "planned": PLANNED,
        "exact_symbols": exact,
