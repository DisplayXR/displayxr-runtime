param([int]$Seconds = 20, [string]$Label = "sample")
...
$names = @{ '0X00024BBF' = 'iGPU'; '0X00024F0B' = 'dGPU'; '0X00024E73' = 'gpu24e73' }
...
foreach ($c in $b) {
    ...
    if ($c.InstanceName -match 'pid_(\d+)_luid_0x[0-9A-Fa-f]+_(0x[0-9A-Fa-f]+)_phys_\d+_eng_\d+_engtype_(\w+)') {
        ...
        $nm = if ($names.ContainsKey($luid)) { $names[$luid] } else { $Matches[2] }
