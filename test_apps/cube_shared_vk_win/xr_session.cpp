...
/*!
 * @file
 * @brief  OpenXR session management for Vulkan with shared D3D11 texture
 *
 * Uses XR_EXT_win32_window_binding with sharedTextureHandle (windowHandle=NULL)
 * for offscreen shared texture compositing via the VK native compositor.
 */
...
bool InitializeOpenXR(XrSessionManager& xr) {
    ...
    if (!xr.hasWin32WindowBindingExt) {
        LOG_ERROR("XR_EXT_win32_window_binding extension required for shared texture mode");
        return false;
    }
    ...
    strcpy_s(createInfo.applicationInfo.applicationName, "SRCubeSharedVK");
    ...
    if (xr.hasDisplayInfoExt) {
        ...
        if (XR_SUCCEEDED(diResult)) {
            ...
            LOG_INFO("Display info: scale=%.3fx%.3f, size=%.3fx%.3fm, pixels=%ux%u",
...
bool CreateVulkanInstance(XrSessionManager& xr, VkInstance& vkInstance) {
    ...
    appInfo.pApplicationName = "SRCubeSharedVK";
...
bool CreateSession(XrSessionManager& xr, VkInstance vkInstance, VkPhysicalDevice physDevice,
    VkDevice device, uint32_t queueFamilyIndex, uint32_t queueIndex, HANDLE sharedTextureHandle)
{
    LOG_INFO("Creating OpenXR session with Vulkan + shared texture...");
