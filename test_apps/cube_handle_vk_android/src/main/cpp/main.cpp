...
// time. They expose `shaders_triangle_vert` / `shaders_triangle_frag` as
...
Mat4
projection_matrix_from_fov(const XrFovf &fov, float near_z, float far_z)
{
...
create_pipeline()
{
	...
	vs_ci.codeSize = sizeof(shaders_triangle_vert);
	vs_ci.pCode = shaders_triangle_vert;
	...
	fs_ci.codeSize = sizeof(shaders_triangle_frag);
	fs_ci.pCode = shaders_triangle_frag;
	...
	if (vkCreateGraphicsPipelines(g_vk_device, VK_NULL_HANDLE, 1, &gpi, nullptr,
