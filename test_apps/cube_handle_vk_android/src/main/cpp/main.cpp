...
record_draw(uint32_t view_idx, uint32_t image_idx, const XrView &view)
{
	...
	// Each eye renders into a HALF-width side-by-side atlas tile (e.g.
	// 1280x1600) that the Leia weave displays across the FULL panel width
	// (2560x1600). So the per-eye aspect as the eye sees it is 2x the tile's
	// pixel aspect. Projecting for the tile aspect (0.8) instead of the
	// displayed aspect (1.6) stretched the cube horizontally AND doubled the
	// stereo disparity (worsening the ghost). The 2.0 factor = the 2x1 SBS
	// horizontal tile count the runtime's atlas uses.
	// 2.0 default = the 2x1 SBS horizontal tile count; live-tunable via
	//   adb shell setprop debug.dxr.aspect_mult <f>   (force-stop + relaunch)
	// so the right value can be dialed in on the panel without rebuilding.
	const float aspect_mult = get_prop_float("debug.dxr.aspect_mult", 2.0f);
	const float aspect = aspect_mult * (float)g_views[view_idx].width / (float)g_views[view_idx].height;
	const Mat4 proj_mat = projection_matrix_from_fov(view.fov, aspect, 0.05f, 100.0f);
