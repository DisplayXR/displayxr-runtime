...
record_draw(uint32_t view_idx, uint32_t image_idx, const XrView &view)
{
	...
	if (g_hud_vbuf != VK_NULL_HANDLE && g_hud_mapped != nullptr) {
		...
		if (hud_n > 0) {
			...
			// Calibrated per display rotation. The compositor already presents
			// content upright for ROTATION_0 (portrait) and ROTATION_90
			// (landscape) — both verified upright at rot=0 via screenshot. The
			// two 180°-flipped orientations need a 180° HUD flip.
			...
			const bool vflip = get_prop_float("debug.dxr.hud_vflip", 0.0f) != 0.0f;
