...
/*!
 * @file
 * @brief  Cube Zones TEXTURE OpenGL — XR_DXR_display_zones parity test (ADR-027), macOS leg
 *
 ...
 * and presents that surface itself, and receives the FULL XR_DXR_display_zones
 ...
 *  - GL graphics binding: XrGraphicsBindingOpenGLMacOSDXR (cglContext from an
 *    OFFSCREEN NSOpenGLContext), with XrCocoaWindowBindingCreateInfoDXR
 ...
 *    XR_DXR_macos_gl_binding is REQUIRED.
...
// see header). It is passed as XrCocoaWindowBindingCreateInfoDXR.viewHandle.
static NSView *g_metalView = nil;
// OFFSCREEN GL context — no on-screen drawable. Its CGLContextObj is passed as
// XrGraphicsBindingOpenGLMacOSDXR.cglContext; the runtime's offscreen GL
...
static bool CreateMacOSWindow(uint32_t width, uint32_t height, int32_t screenLeft, int32_t screenTop)
{
    @autoreleasepool {
        ...
        [g_window setTitle:@"OpenGL Cube Zones TEXTURE — XR_DXR_display_zones parity"];
...
static bool InitializeOpenXR(AppXrSession &app)
{
    ...
    if (!hasMacosGlBinding) {
        LOG_ERROR("Runtime does not support XR_DXR_macos_gl_binding — required for this binding");
        return false;
    }
...
int main(int argc, char **argv)
{
    ...
    LOG_INFO("=== OpenGL Cube Zones TEXTURE OpenXR (XR_DXR_display_zones parity test, ADR-027) ===");
