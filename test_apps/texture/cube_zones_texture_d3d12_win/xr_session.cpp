...
bool InitializeOpenXR(XrSessionManager& xr) {
    ...
    strcpy_s(createInfo.applicationInfo.applicationName, "DXRCubeZonesTextureD3D12");
...
bool CreateSession(XrSessionManager& xr, ID3D12Device* device, ID3D12CommandQueue* queue,
    ...
    LOG_INFO("Creating OpenXR session with D3D12 + shared texture handle + window binding...");
    ...
    // D3D12 binding is required (device + command queue).
    ...
    // Session target extension — chain it to the D3D12 binding. TEXTURE MODE:
    // pass BOTH the shared texture handle (the runtime's composite target) and
    // the app HWND (weaver position tracking). The handle being non-NULL is
    ...
    if (xr.hasWin32WindowBindingExt && appHwnd && sharedTextureHandle) {
        ...
        LOG_INFO("  Chain: XrSessionCreateInfo -> XrGraphicsBindingD3D12KHR -> XrWin32WindowBindingCreateInfoDXR");
    } else {
    ...
    // MUST match `id` in displayxr/cube_zones_texture_d3d12_win.displayxr.json
    ...
    if (xr.hasMcpToolsExt) {
        ...
        if (xr.pfnSetMCPAppInfoEXT && xr.pfnRegisterMCPToolEXT && xr.pfnSubmitMCPToolResultEXT) {
            ...
            strncpy(mcpAppInfo.appId, "cube-zones-texture-d3d12", sizeof(mcpAppInfo.appId) - 1);
