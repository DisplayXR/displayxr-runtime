 ...
 *
 * Port of cube_zones_d3d11_win onto the cube_handle_metal_macos scaffold,
 * then converted to texture class from cube_texture_metal_macos.
 * Exercises the display-zones runtime path (N 3D zones + Local2D zones +
 * per-frame wish mask) end to end on the Metal native compositor:
 *
...
// --- Zone edge fade (content-alpha feather, ADR-027 rule 4) ---
//
// Fullscreen triangle drawn with the viewport set to ONE view tile; blend
// (zero, oneMinusSourceAlpha) on color AND alpha makes dst *= f, f ramping
// 0->1 over the first feather_px from the tile edge.
...
vertex FadeVertexOut fade_vertex(uint id [[vertex_id]])
{
    FadeVertexOut out;
...
fragment float4 fade_fragment(FadeVertexOut in [[stage_in]],
                              constant FadeUniforms &u [[buffer(0)]])
{
    float2 px = in.uv * u.tile_px;
    float d = min(min(px.x, u.tile_px.x - px.x), min(px.y, u.tile_px.y - px.y));
    float f = saturate(d / u.feather_px);
    // Blend (ZERO, INV_SRC_ALPHA): dst *= (1 - src.a) = f.
    return float4(0.0, 0.0, 0.0, 1.0 - f);
}

// --- Blit shader (fullscreen triangle sampling the shared IOSurface) ---
// Texture-mode present: the app reads the runtime-composited IOSurface back
// into its own CAMetalLayer drawable. Lifted from cube_texture_metal_macos.
...
struct MetalRenderer {
    ...
    id<MTLRenderPipelineState> fadePipeline;   //!< zone content-alpha edge fade
    id<MTLRenderPipelineState> blitPipeline;   //!< texture-mode IOSurface -> drawable present
    id<MTLDepthStencilState>   depthState;
    id<MTLDepthStencilState>   depthOffState;  //!< fade pass: depth ignored
...
static bool InitRenderer(MetalRenderer &r)
{
    ...
    // Zone edge-fade pipeline: fullscreen triangle, blend (zero, 1-srcAlpha)
    // on color AND alpha so dst *= f (premultiplied content fade).
    {
        MTLRenderPipelineDescriptor *desc = [[MTLRenderPipelineDescriptor alloc] init];
        desc.vertexFunction = [r.shaderLibrary newFunctionWithName:@"fade_vertex"];
        desc.fragmentFunction = [r.shaderLibrary newFunctionWithName:@"fade_fragment"];
        desc.colorAttachments[0].pixelFormat = MTLPixelFormatBGRA8Unorm;
        ...
        r.fadePipeline = [r.device newRenderPipelineStateWithDescriptor:desc error:&error];
        if (!r.fadePipeline) {
            LOG_ERROR("Fade pipeline creation failed: %s", error.localizedDescription.UTF8String);
            return false;
        }
    }

    // Texture-mode blit pipeline: present the runtime-composited IOSurface
    // into the app's own CAMetalLayer drawable.
    {
        MTLRenderPipelineDescriptor *desc = [[MTLRenderPipelineDescriptor alloc] init];
    ...
    }
    {
        ...
        r.depthOffState = [r.device newDepthStencilStateWithDescriptor:desc];
    }
...
struct DisplayZone {
    ...
    id<MTLTexture> depthTex = nil;  //!< per-zone depth (sizes differ between zones)
};
...
static bool CreateIOSurface(uint32_t width, uint32_t height, id<MTLDevice> device)
{
    ...
    // Read-only texture view for blitting the IOSurface to the app's drawable.
...
{
    // CAMetalLayer needs a device + format to vend drawables. The zones
    // CreateMacOSWindow doesn't set them (the handle app never presents
    // itself), so configure lazily on first present.
    if (metalLayer.device == nil) {
        metalLayer.device = r.device;
...
static void PresentAndMaybeDump(MetalRenderer &renderer)
{
    CAMetalLayer *metalLayer = (CAMetalLayer *)[g_metalView layer];
    if (metalLayer) {
    ...
    // On-demand file-trigger convention (matches the macOS atlas trigger).
    if (access("/tmp/dxr_atlas_trigger", F_OK) == 0) {
        unlink("/tmp/dxr_atlas_trigger");
        DumpIOSurfaceToPNG("/tmp/dxr_atlas.png");
    }
}
...
static bool InitializeOpenXR(AppXrSession &app)
{
    ...
    strncpy(createInfo.applicationInfo.applicationName, "MetalCubeZonesTextureOpenXR",
...
static bool CreateAndFillStrip(AppXrSession &app)
{
    ...
    id<MTLTexture> tex = (__bridge id<MTLTexture>)imgs[idx].texture;
    [tex replaceRegion:MTLRegionMake2D(0, 0, w, h) mipmapLevel:0 withBytes:buf.data() bytesPerRow:stride];
...
// Per-zone depth: zone sizes differ between zones, so the renderer's shared
// depth texture would be recreated every frame if reused here.
static void EnsureZoneDepth(MetalRenderer &r, DisplayZone &z, uint32_t w, uint32_t h)
{
    if (z.depthTex != nil && z.depthTex.width == w && z.depthTex.height == h)
        return;
    MTLTextureDescriptor *desc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatDepth32Float
                                                                                    width:w
                                                                                   height:h
    ...
    desc.usage = MTLTextureUsageRenderTarget;
    desc.storageMode = MTLStorageModePrivate;
    z.depthTex = [r.device newTextureWithDescriptor:desc];
}
...
static void RenderZoneScene(MetalRenderer &r, id<MTLTexture> target, DisplayZone &z,
                            const EyeRenderParams *eyes, int eyeCount)
{
    EnsureZoneDepth(r, z, (uint32_t)target.width, (uint32_t)target.height);
    ...
    rpd.colorAttachments[0].clearColor = MTLClearColorMake(
        z.clearColor[0], z.clearColor[1], z.clearColor[2], z.clearColor[3]);
    rpd.depthAttachment.texture = z.depthTex;
    rpd.depthAttachment.loadAction = MTLLoadActionClear;
    rpd.depthAttachment.storeAction = MTLStoreActionDontCare;
    rpd.depthAttachment.clearDepth = 1.0;
    ...
    if (g_wishMode != 1) {
        ...
        fu.feather_px = kZoneEdgeFadePx;

        [enc setDepthStencilState:r.depthOffState];
        [enc setRenderPipelineState:r.fadePipeline];
...
static void RenderZonesFrame(AppXrSession &app, MetalRenderer &renderer, const XrFrameState &frameState)
{
    ...
    for (uint32_t zi = 0; zi < kNumZones; zi++) {
        ...
        RenderZoneScene(renderer, z.images[imageIndex], z, eyes.data(), (int)n);
