 ...
 *         exerciser — VULKAN leg (hybrid). See xr_session.h.
 */
...
bool InitializeOpenXR(XrSessionManager& xr) {
    ...
    strcpy_s(createInfo.applicationInfo.applicationName, "DXRCubeZonesTextureVK");
...
// ---------------------------------------------------------------------------
// Vulkan setup (mirrors cube_handle_vk_win's xr_session.cpp)
// ---------------------------------------------------------------------------
...
bool CreateVulkanInstance(XrSessionManager& xr, VkInstance& vkInstance) {
    ...
    appInfo.pApplicationName = "DXRCubeZonesTextureVK";
...
bool GetVulkanPhysicalDeviceLUID(VkPhysicalDevice physDevice, LUID& outLuid) {
    // VkPhysicalDeviceIDProperties is core in Vulkan 1.1; chain it into a
    // Properties2 query. deviceLUID is an 8-byte array matching the Windows
    // LUID layout (low DWORD first). deviceLUIDValid must be VK_TRUE.
    ...
    if (!idProps.deviceLUIDValid) {
        LOG_ERROR("VkPhysicalDeviceIDProperties.deviceLUIDValid == FALSE — cannot adapter-match D3D11");
        return false;
    }

    static_assert(sizeof(LUID) == VK_LUID_SIZE, "LUID size mismatch with VK_LUID_SIZE");
    memcpy(&outLuid, idProps.deviceLUID, sizeof(LUID));
    LOG_INFO("Vulkan device LUID: 0x%08lX%08lX (adapter-match target for D3D11 shared texture)",
        (unsigned long)outLuid.HighPart, (unsigned long)outLuid.LowPart);
    return true;
}
    ...
    std::vector<const char*>& deviceExtensions, std::vector<std::string>& extensionStorage)
{
    ...
    // Filter: only request extensions the device actually exposes (extensions
    // promoted to Vulkan core may not be listed).
...
{
    LOG_INFO("Creating OpenXR session with Vulkan + shared texture handle + window binding...");
    ...
    // Session target extension — chain it to the Vulkan binding. TEXTURE MODE:
    // pass BOTH the shared D3D11 texture handle (the runtime's composite
    // target, imported by the VK compositor as a D3D11 KMT texture) and the app
    // HWND (weaver position tracking). The handle being non-NULL is the
    // texture-mode marker (the handle app passes only the HWND).
    ...
    if (xr.hasWin32WindowBindingExt && appHwnd && sharedTextureHandle) {
        // Chain: sessionInfo -> vkBinding -> sessionTarget
        vkBinding.next = &sessionTarget;
        LOG_INFO("Using XR_DXR_win32_window_binding with shared texture + position tracking");
        LOG_INFO("  Chain: XrSessionCreateInfo -> XrGraphicsBindingVulkanKHR -> XrWin32WindowBindingCreateInfoDXR");
    } else {
        LOG_WARN("NOT using XR_DXR_win32_window_binding (hasExt=%d, hwnd=%p, sharedTex=%p)",
            xr.hasWin32WindowBindingExt, (void*)appHwnd, sharedTextureHandle);
    ...
    // XR_DXR_mcp_tools: declare identity + register agent tools. The appId MUST
    // match `id` in displayxr/cube_zones_texture_vk_win.displayxr.json
    ...
    if (xr.hasMcpToolsExt) {
        ...
        if (xr.pfnSetMCPAppInfoEXT && xr.pfnRegisterMCPToolEXT && xr.pfnSubmitMCPToolResultEXT) {
            ...
            strncpy(mcpAppInfo.appId, "cube-zones-texture-vk", sizeof(mcpAppInfo.appId) - 1);
