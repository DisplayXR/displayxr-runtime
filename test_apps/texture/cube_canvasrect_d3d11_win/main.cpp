 ...
 *    reflects what the texture app actually received (see DXR_TEXDUMP below).
 *
...
// stb_image_write is provided by displayxr::common's Windows impl TU
// (atlas_capture.cpp defines STB_IMAGE_WRITE_IMPLEMENTATION). The common stb
// header isn't pulled in here, so forward-declare the single entry point the
// shared-texture readback dump needs. STBIWDEF resolves to `extern "C"` in C++
// (stb_image_write.h), so the symbol has C linkage — match it.
...
static const char* APP_NAME = "cube_canvasrect_d3d11_win";

// Window settings
static const wchar_t* WINDOW_CLASS = L"DXRCubeCanvasRectClass";
static const wchar_t* WINDOW_TITLE = L"D3D11 Cube CanvasRect TEXTURE — XR_DXR_canvas_rect windowless (#697)";
...
// ---------------------------------------------------------------------------
// Texture-mode shared D3D11 texture (lifted from cube_texture_d3d11_win)
// ---------------------------------------------------------------------------
//
...
// region back to its own swapchain. NO output rect / 2D surround needed.
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_WINDOWPOSCHANGED:
        // #697: the window moved or resized — re-declare our on-panel canvas
        // rect so the runtime re-frames (canvas-scoped Kooima) and re-phases the
        // weave. This is the app-owned tracking that replaces the OS's automatic
        // HWND tracking (we passed no HWND to the runtime). No-op until the
        // session + setter exist.
        if (g_xr != nullptr) {
            PushCanvasRect(*g_xr, hwnd);
        }
        break;  // fall through to DefWindowProc so normal move/resize continues
...
// ---------------------------------------------------------------------------
// Texture-mode blit (lifted from cube_texture_d3d11_win)
// ---------------------------------------------------------------------------
...
static bool CreateBlitResources(ID3D11Device* device) {
    ...
    cbd.ByteWidth = 16;  // float2 uvScale + float2 uvOffset, padded to 16
...
static void BlitSharedTextureToBackBuffer(D3D11Renderer& renderer,
    ...
    if (g_blitParamsCB && g_sharedWidth > 0 && g_sharedHeight > 0) {
        float uvParams[4] = {
            vpW / (float)g_sharedWidth, vpH / (float)g_sharedHeight, // uvScale
            0.0f, 0.0f                                               // uvOffset
        };
...
// Autonomous verification: read the shared texture back to a PNG. This is the
// load-bearing proof — it captures exactly what the TEXTURE APP received in
// its shared texture (the full multi-zone composite the runtime wrote back).
// CopyResource into a STAGING texture, Map, swizzle BGRA->RGBA, stbi_write_png.
static void DumpSharedTextureToPNG(D3D11Renderer& renderer, const char* path) {
    ...
    HRESULT hr = renderer.device->CreateTexture2D(&sdesc, nullptr, &staging);
    if (FAILED(hr)) {
        LOG_WARN("TEXTURE READBACK: staging texture create failed (0x%08x) — skipped", hr);
        return;
    }

    renderer.context->CopyResource(staging.Get(), g_sharedTexture.Get());

    D3D11_MAPPED_SUBRESOURCE mapped = {};
    hr = renderer.context->Map(staging.Get(), 0, D3D11_MAP_READ, 0, &mapped);
    if (FAILED(hr)) {
...
struct RenderState {
    ...
    // App-side swapchain + back-buffer RTV for window presentation (texture mode).
...
// (the readback dump reads the shared texture AFTER the present so it captures
// exactly what the texture app received).
static void PresentAndMaybeDump(RenderState& rs) {
    D3D11Renderer& renderer = *rs.renderer;

    // Resize the app swapchain if the window changed.
...
static void RenderZonesFrame(RenderState& rs, const XrFrameState& frameState) {
    ...
    // Texture-mode present + autonomous verification: the runtime composited
    // the full-window multi-zone super-atlas into our shared texture (output
    // rect superseded by zones), so blit it into our own window swapchain and
    // (when DXR_TEXDUMP is set) read it back to a PNG.
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    // Autonomous-verification dump (DXR_TEXDUMP): reads the shared texture
    // back to a PNG at the warmup frame gate. "1" or empty-but-present →
    // default %TEMP%\zones_texture_readback.png; any other value is the
    // literal path.
    {
    ...
    // Create the shared D3D11 texture at the worst-case swapchain atlas dims
    // (same sizing as cube_texture_d3d11_win; see ADR-010). The runtime
    ...
    // Create blit shader resources (present the shared texture to the window).
    ...
    // Release the locals so rs holds the only references — required for
    // ResizeBuffers (the swapchain must have no outstanding back-buffer refs).
