...
/*!
 * @file
 * @brief  OpenGL rendering implementation for cube/grid + display-zones tiles
 *
 * Cloned from cube_zones_texture_vk_win (texture-class D3D11/DComp machinery) +
 * cube_zones_gl_win (GL render + zones logic) for #613. Byte-identical to
 * cube_zones_gl_win's renderer — texture-class lives in main.cpp/xr_session.cpp.
 */
...
// GL port of the D3D11 ZERO/INV_SRC_ALPHA pass: an attribute-less fullscreen
// triangle that outputs alpha = (1 - f) and uses glBlendFuncSeparate(
// GL_ZERO, GL_ONE_MINUS_SRC_ALPHA, ...) so dst *= (1 - (1 - f)) = f. The
// vertex shader writes [-1,1] clip directly from gl_VertexID (no VBO).
static const char* g_fadeVertSrc = R"(
#version 330 core
out vec2 vUV;
void main() {
...
static const char* g_fadeFragSrc = R"(
#version 330 core
    ...
    uint32_t fullWidth, uint32_t fullHeight)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        glBindFramebuffer_(GL_FRAMEBUFFER, zr.fbos[i]);
        glFramebufferTexture2D_(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, images[i], 0);
        glFramebufferRenderbuffer_(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, zr.depthRBO);
        if (glCheckFramebufferStatus_(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
            LOG_ERROR("[zones] zone FBO incomplete for image %u", i);
            return false;
        }
    }
    glBindFramebuffer_(GL_FRAMEBUFFER, 0);
    LOG_INFO("[zones] created %u zone FBOs (%ux%u)", count, fullWidth, fullHeight);
    return true;
}

void DestroyZoneFBOs(GLZoneResources& zr) {
    if (!zr.fbos.empty()) {
        glDeleteFramebuffers_((GLsizei)zr.fbos.size(), zr.fbos.data());
        zr.fbos.clear();
    }
    if (zr.depthRBO) {
        glDeleteRenderbuffers_(1, &zr.depthRBO);
        zr.depthRBO = 0;
    }
    zr.fullW = zr.fullH = 0;
}
...
static void SetMatrix(GLuint program, const char* name, const XMMATRIX& m) {
    ...
    if (loc >= 0) {
        ...
        // GL_FALSE: row-major DirectXMath data read as column-major by GLSL =
        // implicit transpose, matching the HLSL cbuffer behavior.
        glUniformMatrix4fv_(loc, 1, GL_FALSE, &mat._11);
    }
}

// Draw the cube + grid into whatever FBO/viewport is already bound. Shared by
// the fallback RenderScene and the per-tile RenderZoneTile. Assumes depth test
// + cull state already set by the caller.
static void DrawSceneGeometry(
    GLRenderer& renderer,
    const XMMATRIX& viewMatrix,
    const XMMATRIX& projMatrix,
    float zoomScale,
    float cubeY, float cubeZ, float cubeSize)
{
...
void RenderScene(
    ...
    DrawSceneGeometry(renderer, viewMatrix, projMatrix, zoomScale, cubeY, cubeZ, cubeSize);
...
void RenderZoneTile(
    ...
    glEnable(GL_SCISSOR_TEST);
    glEnable(GL_DEPTH_TEST);
    glDepthMask(GL_TRUE);   // depth writes ON so the clear and the cube z-test work
    glDepthFunc(GL_LESS);
    glEnable(GL_CULL_FACE);
    glCullFace(GL_BACK);
    glFrontFace(GL_CCW);
    glDisable(GL_BLEND);    // cube content writes premultiplied-opaque RGBA directly

    // HARD GOTCHA (#613): depth MUST clear every tile, every frame. Color clear
    // is conditional (the caller may have pre-cleared the whole image), but the
    // depth clear is UNCONDITIONAL — otherwise the cube z-fails into a dark
    // untextured shadow (the D3D12 zones bug). Scissor confines both clears to
    // this tile.
    ...
    DrawSceneGeometry(renderer, viewMatrix, projMatrix, 1.0f, cubeY, cubeZ, cubeSize);

    glDisable(GL_SCISSOR_TEST);
    glBindFramebuffer_(GL_FRAMEBUFFER, 0);
}

static bool EnsureEdgeFadePass(GLRenderer& renderer) {
    if (renderer.fadeProgram && renderer.fadeVAO) return true;
    if (renderer.fadeInitTried) return renderer.fadeProgram != 0;
    renderer.fadeInitTried = true;

    renderer.fadeProgram = CreateProgram(g_fadeVertSrc, g_fadeFragSrc);
    if (!renderer.fadeProgram) {
        LOG_ERROR("[zones] edge-fade program compile/link failed");
        return false;
    }
    // Core profile requires a bound (even if empty) VAO for attribute-less draws.
    glGenVertexArrays_(1, &renderer.fadeVAO);
    LOG_INFO("[zones] content-alpha edge fade pass ready");
    return true;
}
...
void DrawZoneEdgeFade(
    ...
    glDisable(GL_CULL_FACE);                // fullscreen tri winding is irrelevant

    // Blend (ZERO, ONE_MINUS_SRC_ALPHA) on both color and alpha: dst *= f.
    glEnable(GL_BLEND);
    glBlendFuncSeparate_(GL_ZERO, GL_ONE_MINUS_SRC_ALPHA, GL_ZERO, GL_ONE_MINUS_SRC_ALPHA);

    glUseProgram_(renderer.fadeProgram);
    GLint tpLoc = glGetUniformLocation_(renderer.fadeProgram, "uTilePx");
    GLint fpLoc = glGetUniformLocation_(renderer.fadeProgram, "uFeatherPx");
    if (tpLoc >= 0) glUniform2f_(tpLoc, (float)tileW, (float)tileH);
    // glUniform1f is not in the loaded pointer set — pass the single feather
    // float through glUniform2f's .x (shader reads uFeatherPx.x).
    if (fpLoc >= 0) glUniform2f_(fpLoc, featherPx, 0.0f);

    glBindVertexArray_(renderer.fadeVAO);
...
void CleanupGLRenderer(GLRenderer& renderer) {
    ...
    if (renderer.fadeVAO) { glDeleteVertexArrays_(1, &renderer.fadeVAO); renderer.fadeVAO = 0; }
    if (renderer.fadeProgram) { glDeleteProgram_(renderer.fadeProgram); renderer.fadeProgram = 0; }
