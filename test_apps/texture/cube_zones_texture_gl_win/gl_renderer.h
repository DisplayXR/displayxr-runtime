 ...
 * cube_zones_gl_win (GL render + zones logic) for #613. This GL renderer is
 * byte-identical to cube_zones_gl_win's — the texture-class difference lives in
 * main.cpp / xr_session.cpp (the shared D3D11 texture + DComp present), NOT in
 * how zone tiles are drawn. The cube/grid renderer is unchanged from the handle app;
