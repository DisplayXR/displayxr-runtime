...
/*!
 * @file
 * @brief  OpenXR session management for the XR_DXR_display_zones TEXTURE exerciser
 */
...
bool CreateSession(XrSessionManager& xr, ID3D11Device* d3d11Device,
    ...
    LOG_INFO("Creating OpenXR session with shared texture handle + window binding...");
    ...
    // Session target extension — chain it to the D3D11 binding. TEXTURE MODE:
    ...
    // MUST match `id` in displayxr/cube_zones_texture_d3d11_win.displayxr.json
    ...
    if (xr.hasMcpToolsExt) {
        ...
        if (xr.pfnSetMCPAppInfoEXT && xr.pfnRegisterMCPToolEXT && xr.pfnSubmitMCPToolResultEXT) {
            ...
            strncpy(mcpAppInfo.appId, "cube-zones-texture-d3d11", sizeof(mcpAppInfo.appId) - 1);
