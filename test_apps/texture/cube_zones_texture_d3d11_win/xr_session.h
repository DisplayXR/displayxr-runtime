 ...
 *
 * Cloned from cube_zones_d3d11_win and converted to TEXTURE class: CreateSession
 * passes the app's shared D3D11 texture HANDLE (the runtime composites into it)
 * + the app HWND (weaver position tracking) via XR_EXT_win32_window_binding —
 * the texture-mode marker. The display-zones detection + entry points are
 * unchanged from cube_zones_d3d11_win. The zones extension composes
...
// runtime's composite target — the texture-mode marker), and the app window
// (weaver position tracking) via XR_EXT_win32_window_binding.
