...
/*!
 * @file
 * @brief  Cube Zones Metal — XR_EXT_display_zones exerciser (ADR-027), macOS leg
 *
...
static bool CreateMacOSWindow(uint32_t width, uint32_t height)
{
    @autoreleasepool {
        ...
        [g_window setTitle:@"Metal Cube Zones — XR_EXT_display_zones"];
...
static bool InitializeOpenXR(AppXrSession &app)
{
    ...
    strncpy(createInfo.applicationInfo.applicationName, "MetalCubeZonesOpenXR",
...
static bool CreateSession(AppXrSession &app, MetalRenderer &r)
{
    ...
    if (g_hasMcpToolsExt) {
        ...
        if (g_pfnSetMCPAppInfo && g_pfnRegisterMCPTool && g_pfnSubmitMCPToolResult) {
            ...
            strncpy(mcpAppInfo.appId, "cube-zones-metal", sizeof(mcpAppInfo.appId) - 1);
...
int main(int argc, char **argv)
{
    ...
    LOG_INFO("=== Metal Cube Zones OpenXR (XR_EXT_display_zones, ADR-027) ===");
                ...
                if (app.pfnCaptureAtlasEXT && app.session != XR_NULL_HANDLE) {
                    if (display3D && (tileColumns > 1 || tileRows > 1)) {
                        std::string prefix = dxr_capture::MakeCaptureAtlasPrefix(
                            "cube_zones_metal_macos", tileColumns, tileRows);
