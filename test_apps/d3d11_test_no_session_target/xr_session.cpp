...
/*!
 * @file
 * @brief  OpenXR session management WITHOUT XR_EXT_session_target
 *
 * This version deliberately does NOT use the session_target extension.
...
bool InitializeOpenXR(XrSessionManager& xr) {
    ...
    LOG_INFO("XR_EXT_session_target: %s (NOT USING - this test skips it)", hasSessionTarget ? "AVAILABLE" : "NOT FOUND");
    ...
    // Build list of extensions to enable - ONLY D3D11, NOT session_target
    std::vector<const char*> enabledExtensions;
    enabledExtensions.push_back(XR_KHR_D3D11_ENABLE_EXTENSION_NAME);
    // NOTE: Deliberately NOT enabling XR_EXT_session_target

    LOG_INFO("Enabling %zu extensions (NO session_target):", enabledExtensions.size());
    ...
    strcpy_s(createInfo.applicationInfo.applicationName, "D3D11TestNoSessionTarget");
...
bool CreateSession(XrSessionManager& xr, ID3D11Device* d3d11Device) {
    ...
    LOG_INFO("  Window handle: NONE (Monado will create its own window)");
    ...
    // Build the session create info - D3D11 binding only, NO session target
    LOG_INFO("Building session create info (D3D11 only, no session_target)...");
    ...
    d3d11Binding.next = nullptr;  // No session target chained

    LOG_INFO("NOT using XR_EXT_session_target - Monado/OpenXR will create its own window");
