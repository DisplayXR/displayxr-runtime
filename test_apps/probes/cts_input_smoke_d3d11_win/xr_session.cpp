...
// XR_EXT_conformance_automation availability (the whole point of this probe).
static bool s_hasConformanceExt = false;

bool HasConformanceAutomationExt() {
    return s_hasConformanceExt;
}
...
bool InitializeOpenXR(XrSessionManager& xr) {
    ...
    for (const auto& ext : extensions) {
        ...
        if (strcmp(ext.extensionName, XR_EXT_CONFORMANCE_AUTOMATION_EXTENSION_NAME) == 0) {
            s_hasConformanceExt = true;
        }
    }
    ...
    if (s_hasConformanceExt) {
        enabledExtensions.push_back(XR_EXT_CONFORMANCE_AUTOMATION_EXTENSION_NAME);
    }
    LOG_INFO("XR_EXT_conformance_automation: %s", s_hasConformanceExt ? "AVAILABLE" : "NOT FOUND");
