...
#include <X11/keysym.h> // XK_m / XK_o for the zones M/O key toggles
...
static bool g_wishModeCycleRequested = false;   // set by the X11 'M' keypress
static bool g_overlapToggleRequested = false;   // set by the X11 'O' keypress
...
static bool CreateAppWindow(uint32_t width, uint32_t height, int32_t screenLeft, int32_t screenTop) {
    ...
    XStoreName(g_xDisplay, g_xWindow, "Cube Zones VK (DisplayXR)");
...
static void PumpAppWindowEvents() {
    ...
    while (XPending(g_xDisplay) > 0) {
        ...
        if (ev.type == ClientMessage &&
            ...
            // Zones controls: M = cycle wish mode (AUTO / explicit Tier-2),
            // O = toggle zone B overlap. Edge-triggered via request flags the
            // per-frame HandleZoneKeys() consumes (mirrors the macOS/Windows
            // zones apps). ESC / Q exit.
            KeySym ks = XLookupKeysym(&ev.xkey, 0);
            if (ks == XK_m || ks == XK_M) {
                g_wishModeCycleRequested = true;
            } else if (ks == XK_o || ks == XK_O) {
                g_overlapToggleRequested = true;
            } else if (ks == XK_Escape || ks == XK_q || ks == XK_Q) {
...
// X11 keypress flags set in PumpAppWindowEvents.
