...
void RenderScene(
    ...
    // Get RTV and DSV handles. rtvOverride (zones ARRAY leg) points at a
    // TEXTURE2DARRAY slice RTV in a zone-owned heap; otherwise index the
    // renderer's swapchain RTV heap. dsvOverride lets a caller bind its own
    // depth; null reuses the shared (atlas-sized) depth buffer.
    D3D12_CPU_DESCRIPTOR_HANDLE rtvHandle;
    if (rtvOverride != nullptr) {
        rtvHandle = *rtvOverride;
    } else {
        rtvHandle = renderer.rtvHeap->GetCPUDescriptorHandleForHeapStart();
        rtvHandle.ptr += rtvIndex * renderer.rtvDescriptorSize;
    }

    D3D12_CPU_DESCRIPTOR_HANDLE dsvHandle = (dsvOverride != nullptr)
        ? *dsvOverride
        : renderer.dsvHeap->GetCPUDescriptorHandleForHeapStart();
    ...
    if (clear) {
        float clearColor[4];
        if (clearColorOverride != nullptr) {
            // Per-zone premultiplied RGBA (zones ARRAY leg).
            clearColor[0] = clearColorOverride[0]; clearColor[1] = clearColorOverride[1];
            clearColor[2] = clearColorOverride[2]; clearColor[3] = clearColorOverride[3];
        } else {
