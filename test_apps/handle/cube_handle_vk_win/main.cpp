...
/*!
 * @file
 * @brief  SR Cube OpenXR Ext VK - OpenXR with XR_DXR_win32_window_binding (Vulkan)
 *
 * Vulkan port of cube_handle_d3d11. Projection layer only, no HUD/quad layer.
 */
...
static const char* APP_NAME = "cube_handle_vk_win";
...
// XR_DXR_view_rig (#396 W7 dogfood): the app chains a rig descriptor
// (XrDisplayRigDXR, or XrCameraRigDXR in camera mode) on every xrLocateViews
...
static HWND CreateAppWindow(HINSTANCE hInstance, int width, int height) {
    ...
    // docs/specs/extensions/XR_DXR_win32_window_binding.md "Transparent-window contract".
            ...
            if (BeginFrame(*xr, frameState)) {
                        ...
                        if (AcquireSwapchainImage(*xr, imageIndex)) {
                            ...
                            for (int eye = 0; eye < eyeCount; eye++) {
                                ...
                                if (useAppProjection) {
                                    ...
                                    eyeParams[eye].viewMatrix = xr->viewMatrices[vi];
                                    eyeParams[eye].projMatrix = xr->projMatrices[vi];
                                }
                            }
                        ...
                        if (rendered && inputSnapshot.hudVisible && hud && xr->hasHudSwapchain && hudSwapchainImages) {
                            ...
                            if (AcquireHudSwapchainImage(*xr, hudImageIndex)) {
                                ...
                                std::wstring modeText = xr->hasWin32WindowBindingExt ?
                                    L"XR_DXR_win32_window_binding: ACTIVE (Vulkan)" :
                                    L"XR_DXR_win32_window_binding: NOT AVAILABLE (Vulkan)";
                ...
                if (g_l2dPanel && !g_l2dActive && rendered &&
                    ...
                    if (!attempted) {
                        ...
                        // panel over the cube and uses the diagonal-stripes
                        // variant (translucent orange between opaque white
                        // stripes) — the clearest read of glass-over-3D.
                        const char* oc = getenv("DXR_LOCAL2D_OVERCUBE");
                        bool overCube = (oc && *oc == '1');
                        int p1variant = overCube ? 1 : 0;
                        if (overCube) {
                            g_panel1Rect.offset = {(int32_t)(winW / 2 - pw / 2),
                                                   (int32_t)(winH / 2 - ph / 2)};
                        } else {
