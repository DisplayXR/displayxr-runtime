...
/*!
 * @file
 * @brief  Metal OpenXR spinning cube with external window (cocoa_window_binding)
 *
 * Demonstrates XR_DXR_cocoa_window_binding with Metal: the app creates its own
 * NSWindow + CAMetalLayer-backed NSView and passes the NSView to the runtime.
 * The runtime renders into the app's window instead of creating its own.
 *
 * Features:
...
static void mat4_from_xr_fov(float m[16], const XrFovf &fov, float nearZ, float farZ)
{
    ...
    m[8]  = (r + l) / w;
    m[9]  = (u + d) / h;
    // Metal uses z range [0, 1] (not OpenGL's [-1, 1])
...
// Model matrix (R | t) from an XrPosef — the forward transform, unlike
// mat4_view_from_xr_pose's inverse. Used for controller markers (#823).
static void mat4_model_from_xr_pose(float m[16], const XrPosef &pose)
{
    float x = pose.orientation.x, y = pose.orientation.y;
    float z = pose.orientation.z, w = pose.orientation.w;
...
// circles) or net_input + scripts/net_input_feeder.py.
// ============================================================================
...
static bool SetupActions(XrInstance instance, XrSession session)
{
    ...
    strncpy(setInfo.actionSetName, "cube_actions", sizeof(setInfo.actionSetName) - 1);
    strncpy(setInfo.localizedActionSetName, "Cube Actions", sizeof(setInfo.localizedActionSetName) - 1);
    ...
    strncpy(ai.actionName, "hand_pose", sizeof(ai.actionName) - 1);
    strncpy(ai.localizedActionName, "Hand Pose", sizeof(ai.localizedActionName) - 1);
    ...
    strncpy(ai.actionName, "select", sizeof(ai.actionName) - 1);
    strncpy(ai.localizedActionName, "Select", sizeof(ai.localizedActionName) - 1);
    ...
    strncpy(ai.actionName, "haptic", sizeof(ai.actionName) - 1);
    strncpy(ai.localizedActionName, "Haptic", sizeof(ai.localizedActionName) - 1);
                         ...
                         const EyeRenderParams *eyes, int eyeCount)
{
    ...
    for (int e = 0; e < eyeCount; e++) {
        ...
        // --- Draw controller markers (#823 actions mode) ---
        if (g_actionsMode) {
            for (int hand = 0; hand < 2; hand++) {
                if (!g_actions.poseValid[hand]) {
                    continue;
                }
                // Small cube at the grip pose; grows while select is held.
                const float scl = g_actions.selectPressed[hand] ? 0.035f : 0.02f;
                float model[16], poseMat[16], scale[16];
                mat4_scaling(scale, scl);
                mat4_model_from_xr_pose(poseMat, g_actions.pose[hand]);
...
// Metal quad pass can be judged on pixels. Default OFF: without it this app's
...
// ============================================================================
// macOS window creation (CAMetalLayer-backed NSView)
// ============================================================================
...
static bool CreateMacOSWindow(uint32_t width, uint32_t height, int32_t screenLeft, int32_t screenTop)
{
    @autoreleasepool {
        ...
        // XrDisplayDesktopPositionDXR); flip into AppKit's bottom-up space.
        ...
        [g_window setTitle:@"Metal Cube — Metal Native Compositor (External Window)"];
    ...
    LOG_INFO("Created macOS window (%ux%u) with CAMetalLayer-backed view", width, height);
    return true;
}
...
{
    LOG_INFO("Creating OpenXR session with Metal binding + cocoa_window_binding...");
    ...
    // session creation (must precede xrAttachSessionActionSets-dependent
    // sync in the frame loop). Failure downgrades to the plain cube.
    if (g_actionsMode && !SetupActions(app.instance, app.session)) {
...
// Fill one static 256x256 BGRA image and hand it to a swapchain (acquire /
...
static bool CreateAndFillQuadTexture(AppXrSession &app, uint32_t size)
{
    ...
    auto put = [&](uint32_t x, uint32_t y, uint8_t b, uint8_t g, uint8_t r, uint8_t a) {
    ...
    for (uint32_t y = 0; y < size; y++) {
        for (uint32_t x = 0; x < size; x++) {
            ...
            if (left && top)          { b = 0;   g = 0;   r = 255; }  // TL red
            else if (right && top)    { b = 40;  g = 210; r = 40;  }  // TR green
            else if (left && bottom)  { b = 255; g = 60;  r = 0;   }  // BL blue
            else if (right && bottom) { b = 20;  g = 220; r = 235; }  // BR yellow
    ...
    id<MTLTexture> tex = (__bridge id<MTLTexture>)imgs[idx].texture;
    [tex replaceRegion:MTLRegionMake2D(0, 0, size, size) mipmapLevel:0 withBytes:buf bytesPerRow:stride];
    free(buf);
...
int main(int argc, char **argv)
{
    ...
    // #823 actions mode: `--actions` flag or DXR_ACTIONS=1.
    ...
    {
        const char *e = getenv("DXR_ACTIONS");
    ...
    if (g_actionsMode) {
        LOG_INFO("Actions mode ENABLED (--actions): khr/simple_controller pose+select+haptic; "
                 "controller markers render as small cubes");
    }
    ...
    while (g_running && !app.exitRequested) {
        ...
        if (g_quadTest && !g_quadActive && g_frameCounter >= g_l2dActivationFrame) {
        ...
        // #823 actions mode: sync + locate the controllers for this frame's
        // display time; the renderer picks the results up from g_actions.
        if (g_actionsMode) {
            UpdateActions(app.session, app.localSpace, frameState.predictedDisplayTime);
        }
        ...
        if (g_quadTest && g_quadActive && viewCount >= 1 && g_renderW > 0) {
            ...
            if (g_frameCounter > 60 && quadExpectLogged < 2) {
                ...
                for (uint32_t v = 0; v < viewCount && v < 8; v++) {
                    ...
                    // Vulkan-vs-Metal/D3D clip-Y convention slip hides, and an
        ...
        if (hudSubmitted) {
            ...
            if (layerCount > 0 && g_quadActive && g_quadSwapchain != XR_NULL_HANDLE) {
                ...
                // (this box: ~28 deg horizontal over a 0.44 x 0.24 m canvas)
                ...
                // oxr_session_frame_end's handle_space HEAD-RELATIVE ("poses
                // in view space are already in the space the compositor
                // expects"), but every other layer pose — and the projection
                // view poses the camera is built from — is resolved into the
                // head xdev's TRACKING frame, in which the head sits ~1.6 m
                // up. So a VIEW-space quad lands ~1.6 m below everything else
                // and falls straight out of the frustum. Confirmed here with
                // the runtime's own layer dump. The D3D11 renderer has the
                // identical gap (it does not read
                // XRT_LAYER_COMPOSITION_VIEW_SPACE_BIT either), so this is a
                // pre-existing cross-backend space bug, not a #1581
                // regression, and it belongs with #1580's camera/space work.
                // Using VIEW here would test that bug instead of the quad pass.
