...
bool InitializeOpenXR(XrSessionManager& xr) {
    ...
    // XR_DXR_atlas_capture (#396 W6): resolve the runtime-owned capture entry.
    if (xr.hasAtlasCaptureExt) {
...
bool CreateSession(XrSessionManager& xr, ID3D12Device* device, ID3D12CommandQueue* queue, HWND hwnd) {
    LOG_INFO("Creating OpenXR session with D3D12 + XR_DXR_win32_window_binding...");
    ...
    // XR_DXR_display_zones entry points (ADR-027): capabilities + per-zone
    ...
    // appId MUST match `id` in displayxr/cube_zones_d3d12_win.displayxr.json
    ...
    if (xr.hasMcpToolsExt) {
        ...
        if (xr.pfnSetMCPAppInfoEXT && xr.pfnRegisterMCPToolEXT && xr.pfnSubmitMCPToolResultEXT) {
            ...
            strncpy(mcpAppInfo.appId, "cube-zones-d3d12", sizeof(mcpAppInfo.appId) - 1);
