...
/*!
 * @file
 * @brief  OpenXR session management for the XR_DXR_display_zones exerciser
 */
...
bool InitializeOpenXR(XrSessionManager& xr) {
    ...
    strcpy_s(createInfo.applicationInfo.applicationName, "DXRCubeZones");
...
bool CreateSession(XrSessionManager& xr, ID3D11Device* d3d11Device, HWND hwnd) {
    ...
    // MUST match `id` in displayxr/cube_zones_d3d11_win.displayxr.json
    ...
    if (xr.hasMcpToolsExt) {
        ...
        if (xr.pfnSetMCPAppInfoEXT && xr.pfnRegisterMCPToolEXT && xr.pfnSubmitMCPToolResultEXT) {
            ...
            strncpy(mcpAppInfo.appId, "cube-zones-d3d11", sizeof(mcpAppInfo.appId) - 1);
