...
bool InitializeOpenXR(XrSessionManager& xr) {
    ...
    strcpy_s(createInfo.applicationInfo.applicationName, "DXRCubeZonesGL");
    ...
    if (xr.hasDisplayInfoExt) {
        ...
        // Load xrRequestDisplayRenderingModeDXR function pointer
    ...
    //
    // GL Tier-3 STUB: xrAcquireLocal3DZoneRenderTargetDXR is deliberately NOT
    ...
    // AUTO (matching the Vulkan zones app).
    if (g_zone.available) {
...
bool CreateSession(XrSessionManager& xr, HDC hDC, HGLRC hGLRC, HWND hwnd) {
    LOG_INFO("Creating OpenXR session with OpenGL + XR_DXR_win32_window_binding...");
    ...
    // opts out. Mirrors the D3D11 zones app's default-flipped plumbing.
    {
    ...
    if (xr.hasWin32WindowBindingExt && hwnd) {
        ...
        LOG_INFO("  Chain: XrSessionCreateInfo -> XrGraphicsBindingOpenGLWin32KHR -> XrWin32WindowBindingCreateInfoDXR");
    } else {
    ...
    // match `id` in displayxr/cube_zones_gl_win.displayxr.json (INV-10.1).
    ...
    if (xr.hasMcpToolsExt) {
        ...
        if (xr.pfnSetMCPAppInfoEXT && xr.pfnRegisterMCPToolEXT && xr.pfnSubmitMCPToolResultEXT) {
            ...
            strncpy(mcpAppInfo.appId, "cube-zones-gl", sizeof(mcpAppInfo.appId) - 1);
