...
struct xrt_display_processor_metal
{
	 ...
	 * Decision 5 — Metal port of the D3D11 slot). The caller pre-sets
	 ...
	 */
	bool (*get_local_zone_caps)(struct xrt_display_processor_metal *xdp,
	                            struct xrt_dp_local_zone_caps *out_caps);

	/*!
	 * Publish this client's screen-anchored 3D-zone mask (#224 Phase 0;
	 * wish semantics per ADR-027 Decision 5 — Metal port of the D3D11
	 * slot, mask handle following this header's @ref set_background_2d
	 * id<MTLTexture> convention).
	 *
	 * The runtime owns the mask texture (r8Unorm, client-window pixels)
	 * and passes it here with all producing GPU/CPU work committed and
	 * completed — the DP samples or copies it DURING this call (on its own
	 * command buffer if it needs GPU access) and must not hold the pointer
	 * past return. @p screen_x/y/w/h anchor the mask's pixel space on the
	 * panel in physical screen pixels (post-DPI client rect). @p seq is
	 * the mask CONTENT generation — monotonic, bumped only when the
	 * published content changes (xrSubmitLocal3DZoneDXR, a wish re-raster,
	 * an explicit-wish change); same-seq publishes differ only in the
	 * screen anchor, so a vendor evaluates content once per generation.
	 *
	 ...
	 *
	 * @param xdp           Pointer to self.
	 * @param mask_texture  Mask texture (id<MTLTexture>, r8Unorm).
	 ...
	 */
	bool (*publish_local_zone_mask)(struct xrt_display_processor_metal *xdp,
	                                void *mask_texture,
	                                uint32_t mask_width,
	 ...
	 */
	bool (*clear_local_zone_mask)(struct xrt_display_processor_metal *xdp);
...
/*!
 * Defined when this header carries the @ref xrt_display_processor_metal::get_scanout_caps
 ...
 */
#define XRT_DP_METAL_HAS_SCANOUT_CAPS 1
...
/*!
 * @copydoc xrt_display_processor_metal::get_local_zone_caps
 ...
 */
static inline bool
xrt_display_processor_metal_get_local_zone_caps(struct xrt_display_processor_metal *xdp,
                                                struct xrt_dp_local_zone_caps *out_caps)
{
...
/*!
 * @copydoc xrt_display_processor_metal::publish_local_zone_mask
 * Returns false if not supported (slot absent or NULL).
 * @public @memberof xrt_display_processor_metal
 */
static inline bool
xrt_display_processor_metal_publish_local_zone_mask(struct xrt_display_processor_metal *xdp,
                                                    void *mask_texture,
                                                    uint32_t mask_width,
                                                    uint32_t mask_height,
                                                    int32_t screen_x,
                                                    ...
                                                    uint64_t seq)
{
	...
	return xdp->publish_local_zone_mask(xdp, mask_texture, mask_width, mask_height, screen_x, screen_y, screen_w,
	                                    screen_h, seq);
}

/*!
 * @copydoc xrt_display_processor_metal::clear_local_zone_mask
 * Returns false if not supported (slot absent or NULL).
 * @public @memberof xrt_display_processor_metal
 */
static inline bool
xrt_display_processor_metal_clear_local_zone_mask(struct xrt_display_processor_metal *xdp)
{
...
/*!
 * @copydoc xrt_display_processor_metal::get_scanout_caps
 *
 ...
 * @ref xrt_display_processor_metal_get_weave_scope, which does that for you.
 *
 * @public @memberof xrt_display_processor_metal
 */
static inline bool
xrt_display_processor_metal_get_scanout_caps(struct xrt_display_processor_metal *xdp,
...
xrt_display_processor_metal_get_weave_scope(struct xrt_display_processor_metal *xdp)
{
	struct xrt_dp_scanout_caps caps;
	xrt_dp_scanout_caps_init(&caps);
	if (!xrt_display_processor_metal_get_scanout_caps(xdp, &caps)) {
