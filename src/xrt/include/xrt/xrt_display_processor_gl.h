...
struct xrt_display_processor_gl
{
	 ...
	 * @param      format           GL internal format of the atlas texture (GLenum as
	 ...
	 */
	enum xrt_dp_color_capability (*get_handoff_color_capability)(struct xrt_display_processor_gl *xdp);
	 ...
	 * Decision 5 — GL port of the D3D11 slot). The caller pre-sets
	 ...
	 */
	bool (*get_local_zone_caps)(struct xrt_display_processor_gl *xdp,
	                            struct xrt_dp_local_zone_caps *out_caps);

	/*!
	 * Publish this client's screen-anchored 3D-zone mask (#224 Phase 0;
	 * wish semantics per ADR-027 Decision 5 — GL port of the D3D11 slot,
	 * mask handle following this header's @ref set_background_2d GLuint
	 * convention).
	 *
	 * The runtime owns the mask texture (R8, client-window pixels) and
	 * passes its name here; the call runs with the runtime's GL context
	 * current (GL context is thread-local state, matching @ref
	 * process_atlas) and all GL commands producing the texture already
	 * issued on that context — the DP samples or copies it DURING this
	 * call and must not hold the name past return. @p screen_x/y/w/h
	 * anchor the mask's pixel space on the panel in physical screen pixels
	 * (post-DPI client rect). @p seq is the mask CONTENT generation —
	 * monotonic, bumped only when the published content changes
	 * (xrSubmitLocal3DZoneDXR, a wish re-raster, an explicit-wish change);
	 * same-seq publishes differ only in the screen anchor, so a vendor
	 * evaluates content once per generation.
	 *
	 ...
	 *
	 * @param xdp          Pointer to self.
	 * @param mask_tex     Mask texture name (GLuint, R8).
	 ...
	 */
	bool (*publish_local_zone_mask)(struct xrt_display_processor_gl *xdp,
	                                uint32_t mask_tex,
	                                uint32_t mask_width,
	 ...
	 */
	bool (*get_backend_state)(struct xrt_display_processor_gl *xdp, uint32_t *out_state);
	 ...
	 * @return true if @p out_caps was filled.
	 */
	bool (*get_scanout_caps)(struct xrt_display_processor_gl *xdp, struct xrt_dp_scanout_caps *out_caps);
	 ...
	 */
	bool (*get_background_preview)(struct xrt_display_processor_gl *xdp,
	                               struct xrt_dp_background_preview *out_preview);
};


/*!
 * Defined when this header carries the @ref xrt_display_processor_gl::get_scanout_caps
 ...
 */
#define XRT_DP_GL_HAS_SCANOUT_CAPS 1
...
XRT_DP_ABI_ASSERT(offsetof(struct xrt_display_processor_gl, get_backend_state)           == XRT_DP_GL_BASE_OFF + 19 * sizeof(void *), XRT_DP_ABI_MSG);
XRT_DP_ABI_ASSERT(offsetof(struct xrt_display_processor_gl, get_scanout_caps)            == XRT_DP_GL_BASE_OFF + 20 * sizeof(void *), XRT_DP_ABI_MSG);
XRT_DP_ABI_ASSERT(offsetof(struct xrt_display_processor_gl, get_background_preview) == XRT_DP_GL_BASE_OFF + 21 * sizeof(void *), XRT_DP_ABI_MSG);
XRT_DP_ABI_ASSERT(sizeof(struct xrt_display_processor_gl)                                == XRT_DP_GL_BASE_OFF + 22 * sizeof(void *), XRT_DP_ABI_MSG);
 ...
 */
#define XRT_DP_GL_HAS_BACKGROUND_PREVIEW 1
 ...
 * coupled-ABI-addition pattern (see XRT_DP_VK_HAS_PRESENT_ORIGIN).
 */
#define XRT_DP_GL_HAS_BACKEND_STATE 1
...
/*!
 * @copydoc xrt_display_processor_gl::get_handoff_color_capability
 * Returns @ref XRT_DP_COLOR_ENCODED if not supported (slot absent or NULL).
 * @public @memberof xrt_display_processor_gl
 */
static inline enum xrt_dp_color_capability
xrt_display_processor_gl_get_handoff_color_capability(struct xrt_display_processor_gl *xdp)
{
...
/*!
 * @copydoc xrt_display_processor_gl::set_atlas_encoding
 * No-op if not supported (slot absent or NULL) — the DP then assumes ENCODED.
 * @public @memberof xrt_display_processor_gl
 */
static inline void
xrt_display_processor_gl_set_atlas_encoding(struct xrt_display_processor_gl *xdp,
...
/*!
 * @copydoc xrt_display_processor_gl::get_local_zone_caps
 ...
 */
static inline bool
xrt_display_processor_gl_get_local_zone_caps(struct xrt_display_processor_gl *xdp,
                                             struct xrt_dp_local_zone_caps *out_caps)
{
...
/*!
 * @copydoc xrt_display_processor_gl::publish_local_zone_mask
 * Returns false if not supported (slot absent or NULL).
 * @public @memberof xrt_display_processor_gl
 */
static inline bool
xrt_display_processor_gl_publish_local_zone_mask(struct xrt_display_processor_gl *xdp,
                                                 uint32_t mask_tex,
                                                 uint32_t mask_width,
                                                 uint32_t mask_height,
                                                 int32_t screen_x,
                                                 ...
                                                 uint64_t seq)
{
	...
	return xdp->publish_local_zone_mask(xdp, mask_tex, mask_width, mask_height, screen_x, screen_y, screen_w,
	                                    screen_h, seq);
}

/*!
 * @copydoc xrt_display_processor_gl::clear_local_zone_mask
 * Returns false if not supported (slot absent or NULL).
 * @public @memberof xrt_display_processor_gl
 */
static inline bool
xrt_display_processor_gl_clear_local_zone_mask(struct xrt_display_processor_gl *xdp)
{
...
/*!
 * @copydoc xrt_display_processor_gl::get_backend_state
 * Returns false if not supported (slot absent or NULL) — the caller then
 * treats the backend state as unknown, i.e. @ref XRT_DP_BACKEND_STATE_OK.
 * @public @memberof xrt_display_processor_gl
 */
static inline bool
xrt_display_processor_gl_get_backend_state(struct xrt_display_processor_gl *xdp, uint32_t *out_state)
{
...
/*!
 * @copydoc xrt_display_processor_gl::get_scanout_caps
 *
 ...
 * @ref xrt_display_processor_gl_get_weave_scope, which does that for you.
 *
 * @public @memberof xrt_display_processor_gl
 */
static inline bool
xrt_display_processor_gl_get_scanout_caps(struct xrt_display_processor_gl *xdp, struct xrt_dp_scanout_caps *out_caps)
{
...
xrt_display_processor_gl_get_weave_scope(struct xrt_display_processor_gl *xdp)
{
	struct xrt_dp_scanout_caps caps;
	xrt_dp_scanout_caps_init(&caps);
	if (!xrt_display_processor_gl_get_scanout_caps(xdp, &caps)) {
...
/*!
 * @copydoc xrt_display_processor_gl::get_background_preview
 *
 ...
 */
static inline bool
xrt_display_processor_gl_get_background_preview(struct xrt_display_processor_gl *xdp,
                                                struct xrt_dp_background_preview *out_preview)
{
