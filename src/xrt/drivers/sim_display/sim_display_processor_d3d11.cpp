...
struct sim_display_processor_d3d11_impl
{
	...
	ID3D11PixelShader *ps_shaders[4]; //!< One per output mode (SBS, anaglyph, blend, squeezed SBS)
                                   ...
                                   struct xrt_display_processor_d3d11 **out_xdp)
{
	...
	// Compile all 4 pixel shaders so runtime switching is instant
	...
	U_LOG_W("Created sim display D3D11 processor (all 4 shaders), initial mode: %s",
