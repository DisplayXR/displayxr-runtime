...
struct sim_display_processor_d3d12_impl
{
	...
	//! #224 / ADR-027 local-zone test double (D3D12 port of the D3D11
...
/*
 *
 * #224 / ADR-027 local 2D/3D zones — D3D12 port of the D3D11 test double.
 *
 ...
 * published ID3D12Resource would need a readback heap + command
 * allocator/list/fence this test double doesn't carry (its process_atlas
 * only records onto the caller's list) — the D3D11 and GL variants provide
 * the real content dumps.
 *
 */

static bool
sim_dp_d3d12_get_local_zone_caps(struct xrt_display_processor_d3d12 *xdp, struct xrt_dp_local_zone_caps *out_caps)
{
	struct sim_display_processor_d3d12_impl *sdp = sim_dp_d3d12(xdp);
...
static bool
sim_dp_d3d12_publish_local_zone_mask(struct xrt_display_processor_d3d12 *xdp,
                                     void *mask_resource,
                                     uint32_t mask_width,
...
{
	struct sim_display_processor_d3d12_impl *sdp = sim_dp_d3d12(xdp);
	if (mask_resource == nullptr || mask_width == 0 || mask_height == 0) {
		return false;
	}
	...
	if (geo_changed) {
		U_LOG_W("SIM ZONE PUBLISH (D3D12): mask=%ux%u screen=(%d,%d %ux%u) grid=%ux%u seq=%llu", mask_width,
		...
		if (sdp->zone_cfg.dump) {
			...
			if (!dump_noted) {
				dump_noted = true;
				U_LOG_W("SIM ZONE DUMP (D3D12): log-only on this variant (no readback-heap path) — "
...
static bool
sim_dp_d3d12_clear_local_zone_mask(struct xrt_display_processor_d3d12 *xdp)
{
	struct sim_display_processor_d3d12_impl *sdp = sim_dp_d3d12(xdp);
	if (sdp->zone_active) {
		U_LOG_W("SIM ZONE CLEAR (D3D12): client contribution withdrawn (last seq=%llu)",
