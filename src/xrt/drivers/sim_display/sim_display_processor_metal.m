...
static NSString *const shader_source = @
    ...
    "}\n"
    "\n"
    "// Passthrough: first tile (view 0) fills the entire screen.\n"
    "fragment float4 passthrough_fragment(VertexOut in [[stage_in]],\n"
    "                                     texture2d<float> tex [[texture(0)]],\n"
    "                                     sampler smp [[sampler(0)]],\n"
    "                                     constant TileParams &tp [[buffer(0)]]) {\n"
    "    float2 atlas_uv = float2(in.texCoord.x * tp.tile_cols_inv,\n"
    "                             in.texCoord.y * tp.tile_rows_inv);\n"
    "    return tex.sample(smp, atlas_uv);\n"
    "}\n";
...
struct sim_display_processor_metal
{
	...
	//! #224 / ADR-027 local-zone test double (Metal port of the D3D11
                            ...
                            uint32_t canvas_height)
{
	...
	// 3D shaders (anaglyph, SBS, blend) need ≥2 views.
...
/*
 *
 * #224 / ADR-027 local 2D/3D zones — Metal port of the D3D11 test double
 * (CODE-ONLY: needs a Mac eyeball, mirrors the in-file log idioms).
 *
 ...
 * SIM_DISPLAY_ZONE_DUMP is LOG-ONLY on this variant: the published
 * id<MTLTexture> is typically private/GPU storage, so a CPU readback would
 * need a blit to a shared buffer + a command buffer round-trip this test
 * double doesn't carry — the D3D11 and GL variants provide the real content
 * dumps.
 *
 */

static bool
sim_dp_metal_get_local_zone_caps(struct xrt_display_processor_metal *xdp, struct xrt_dp_local_zone_caps *out_caps)
{
	struct sim_display_processor_metal *sdp = sim_dp_metal(xdp);
...
static bool
sim_dp_metal_publish_local_zone_mask(struct xrt_display_processor_metal *xdp,
                                     void *mask_texture,
                                     uint32_t mask_width,
...
{
	struct sim_display_processor_metal *sdp = sim_dp_metal(xdp);
	if (mask_texture == NULL || mask_width == 0 || mask_height == 0) {
		return false;
	}
	...
	if (geo_changed) {
		U_LOG_W("SIM ZONE PUBLISH (Metal): mask=%ux%u screen=(%d,%d %ux%u) grid=%ux%u seq=%llu", mask_width,
		...
		if (sdp->zone_cfg.dump) {
			...
			if (!dump_noted) {
				dump_noted = true;
				U_LOG_W("SIM ZONE DUMP (Metal): log-only on this variant (no blit readback path) — "
...
static bool
sim_dp_metal_clear_local_zone_mask(struct xrt_display_processor_metal *xdp)
{
	struct sim_display_processor_metal *sdp = sim_dp_metal(xdp);
	if (sdp->zone_active) {
		U_LOG_W("SIM ZONE CLEAR (Metal): client contribution withdrawn (last seq=%llu)",
                                   ...
                                   struct xrt_display_processor_metal **out_xdp)
{
	...
	sdp->base.get_display_dimensions = sim_dp_metal_get_display_dimensions;   // #856
	sdp->base.get_display_pixel_info = sim_dp_metal_get_display_pixel_info;   // #856
