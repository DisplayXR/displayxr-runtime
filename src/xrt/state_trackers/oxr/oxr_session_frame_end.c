                             ...
                             struct xrt_device *head)
{
	...
	// conformance run on the exact-2 path (a CTS session never enables this
	// extension, and sim-display sits in a 1-view mode for most of the suite).
#ifdef OXR_HAVE_DXR_display_info
	const bool display_info_enabled = sess->sys->inst->extensions.DXR_display_info;
#else
	const bool display_info_enabled = false;
#endif
	...
	if (is_multiview || sess->view_config_type == XR_VIEW_CONFIGURATION_TYPE_PRIMARY_STEREO) {
		...
		const enum oxr_view_count_verdict verdict = oxr_projection_view_count_verdict(
		    proj->viewCount, reported, is_multiview, active_mode_view_count, display_info_enabled,
		    mode_view_counts, mode_count, oxr_under_submit_setting());
