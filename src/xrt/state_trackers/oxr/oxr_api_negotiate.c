...
handle_non_null(struct oxr_instance *inst, struct oxr_logger *log, const char *name, PFN_xrVoidFunction *out_function)
{
	...
	ENTRY_IF_EXT(xrSetWaylandSurfaceGeometryDXR, DXR_wayland_surface_binding);
#endif
	...
	ENTRY_IF_EXT(xrWeaveSnapWindowGridDXR, DXR_weave);
	...
	ENTRY_IF_EXT(xrGetLiftPropertiesDXR, DXR_lift);
	ENTRY_IF_EXT(xrCreateLiftStreamDXR, DXR_lift);
	ENTRY_IF_EXT(xrDestroyLiftStreamDXR, DXR_lift);
