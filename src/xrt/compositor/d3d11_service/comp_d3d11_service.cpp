...
after_key_shortcuts:
	...
	// 3D-sprite render are the workspace controller's job (ADR-018 / PR 6
	// of the runtime→controller migration). The OS cursor is hidden over
	// the workspace HWND via WM_SETCURSOR -> SetCursor(NULL).
	//
	...
	if (mc->window != nullptr) {
		...
		// onnect, so FOCUS_CHANGED reaches the shell promptly.
	...
	// 3D cursor sprite is the workspace controller's job (ADR-018 / PR 6
	// of the runtime→controller migration). The controller submits a
	// chrome swapchain for its cursor and drives per-frame pose via
	// xrUpdateWorkspaceClientChromeLayerPoseEXT (spec_version 12, PR 5).
	// The OS cursor is already hidden over the workspace HWND via
	// WM_SETCURSOR -> SetCursor(NULL) in comp_d3d11_window.cpp.
