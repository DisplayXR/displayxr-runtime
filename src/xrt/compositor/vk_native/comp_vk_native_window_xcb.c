                                                ...
                                                int32_t *out_top_px)
{
	...
	// Find the window's root (translate target) via its geometry reply.
	xcb_get_geometry_cookie_t gc = xcb_get_geometry(conn, (xcb_drawable_t)handle->window);
	xcb_get_geometry_reply_t *gr = xcb_get_geometry_reply(conn, gc, NULL);
	if (gr == NULL) {
		return false;
	}
	xcb_window_t root = gr->root;
	free(gr);

	// Window origin translated into root (screen) coordinates.
	xcb_translate_coordinates_cookie_t cookie =
	    xcb_translate_coordinates(conn, (xcb_window_t)handle->window, root, 0, 0);
