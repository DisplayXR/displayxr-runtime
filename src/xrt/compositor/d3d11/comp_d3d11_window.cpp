...
wnd_proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SETCURSOR:
		...
		// The workspace controller renders its own 3D cursor sprite via
		// a chrome swapchain + xrUpdateWorkspaceClientChromeLayerPoseEXT
		// (spec_version 12, PR 5/6 of the runtime->controller migration,
		// ADR-018). Win32 keeps tracking the cursor position for hit-test,
		// drag operations, and event delivery — only the visual is
		// suppressed.
