...
/*!
 * KHR cube / cylinder / equirect(1,2) layers (#1544).
 *
 * The GL lane advertises the KHR layer extensions instance-wide: this build
                         ...
                         const struct xrt_layer_data *data)
{
	...
	if (!noted) {
		U_LOG_I("GL compositor: cube layers are accepted but not drawn (#1544)");
		noted = true;
	}
...
static xrt_result_t
gl_compositor_layer_cylinder(struct xrt_compositor *xc,
                             struct xrt_device *xdev,
                             struct xrt_swapchain *xsc,
                             const struct xrt_layer_data *data)
{
	...
	static bool noted = false;
	if (!noted) {
		U_LOG_I("GL compositor: cylinder layers are accepted but not drawn (#1544)");
		noted = true;
	}
...
static xrt_result_t
gl_compositor_layer_equirect1(struct xrt_compositor *xc,
                              struct xrt_device *xdev,
                              struct xrt_swapchain *xsc,
                              const struct xrt_layer_data *data)
{
	...
	static bool noted = false;
	if (!noted) {
		U_LOG_I("GL compositor: equirect1 layers are accepted but not drawn (#1544)");
		noted = true;
	}
...
static xrt_result_t
gl_compositor_layer_equirect2(struct xrt_compositor *xc,
                              struct xrt_device *xdev,
                              struct xrt_swapchain *xsc,
                              const struct xrt_layer_data *data)
{
                          ...
                          struct xrt_compositor_native **out_xcn)
{
	...
	// #1544: the GL lane advertises the KHR layer extensions instance-wide;
	// a NULL slot must fail the layer, not the process.
	xc->layer_cube = gl_compositor_layer_cube;
	xc->layer_cylinder = gl_compositor_layer_cylinder;
	xc->layer_equirect1 = gl_compositor_layer_equirect1;
	xc->layer_equirect2 = gl_compositor_layer_equirect2;
